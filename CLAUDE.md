...
- **Iconography ships as text glyphs; the sprite atlas is post-v1 (§5.6.7).** What is built and is the v1 answer: a two-palette **text-glyph** icon set — Nerd Fonts v3 when `ui.nerd_fonts=on`, BMP-block fallback otherwise — across the file tree, oil, listings, LSP severity marks, completion-candidate kinds, the references picker, multibuffer headers and the compilation headerline. What is **not** built and is fenced 📝 in design.md §5.6.7: the raster **sprite atlas** (SVG/PNG, plugin-registered `SpriteSet`s, user TOML overrides). Both palettes must occupy the same cell width — see the icon-degradation rule under "UX rules". Path 4 (full inline media blocks) stays post-1.0. Known small gaps in the text-glyph set: notification badges and tab-strip language logos.
...
- **Synthetic buffers are Documents with a name.** LSP logs, `*messages*`, future `*scratch*` go through the buffer registry, `:ls`, `:b <name>`, default status-line and save paths as Document buffers with a synthetic name slot. They are read-only from the user's perspective forever (no toggle-off-read-only path); subsystem owners write via the document handle's `apply_edit_batch` and so bypass the gate without a capability token. **Making one read-only takes TWO declarations, not one.** `ReadOnly = true` is read by `read_only_edit_rejected`, which guards the insert-mode char path and nothing else — operators never reach it, because a `Document`'s grammar dispatch applies its own edits and hands the host an already-applied `Effect::Edits`. So a mode must ALSO `implies` → `read-only-mode`, which carries the `invocation_runner` (`Editor::run_read_only_motion`) that refuses `x` / `dd` / `cw` / `p`. Declare it on the MAJOR: an implied mode is followed from the mode being activated, so putting it on a shared minor looks tidier and does not fire. This paragraph previously said the option alone enforced at "the dispatcher's modal Insert / operators path"; it does not, and magit, `*compilation*`, `*plugin-trace*` and `*messages*` all shipped editable as a result. Buffer KINDS that are `is_read_only()` (Help, FileTree, Multibuffer, Terminal, Dashboard) are gated by kind and need none of this. Help-mode features (syntax highlight, link rendering) may be reused but the buffer is not a HelpBuffer. The Group-1 set (`:help`, `:describe-*`, `:apropos`, `:list-*`, `:keymap`, `:options`, `:diagnostics`, `:lsp-status`) genuinely uses help-mode features (links, anchors, dismiss-on-Esc) and stays as HelpBuffer.
...
A slice plan moves to `docs/dev/operations/slice-plans/archive/` **only when every one of its slices is settled — each either complete (✅) or dropped (❌)**. A plan with even one slice that is in-progress (🚧), planned (📝), or **deferred (⛔)** stays active — it is NOT archived. **Deferred is not complete:** a deferred slice is open work (postponed to a later phase, blocked, or accepted-later polish), and keeping its plan active is what keeps that work visible. Archiving a plan because "only the deferred bits are left" is the specific mistake this rule prevents (it buried modeline's `ML.4`/`ML.6` and would have buried dashboard `DB.8`, plugin-host `PH7.4e`, acp-ux `PU-B.2b-iv`).

**⛔ deferred and ❌ dropped are different states and only one of them blocks.** Deferred means *not yet* — the work is still wanted and something names when it comes back ("revisit if fragment parsing proves visibly wrong on real code"). Dropped means *not at all* — a decision was made and there is nothing left to do: superseded by another slice, made moot by an upstream change, or simply declined. Dropped work is settled work, so it does not keep a plan out of the archive.

The distinction was invisible before 2026-09-09 and the plans already carried it in prose — `IN.10 ⛔ DROPPED`, `MV.2 ⛔ dropped`, `LR.4 Superseded ⛔`, `OA.17 dropped, org removed it in 9.1` — all wearing the same icon as genuine deferrals. That made the archiving rule read the wrong signal, and a rule that depends on prose rather than on a marker cannot be grepped and drifts. **When marking something ❌, say in one line what decided it**; "dropped" with no reason is indistinguishable from abandoned-and-forgotten, which is the state this icon exists to rule out.
