...
// SS.1 (2026-08-11): the fingerprint moved down to `lattice-core` so
// `lattice-multibuffer` can guard its own source writes with the SAME
// mechanism. Re-exported here so autoread's callers are unchanged.
pub use lattice_core::on_disk::OnDiskFingerprint;
