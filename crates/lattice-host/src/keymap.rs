...
// K.2.2 (2026-06-01): `BindingMode` moved to `lattice-mode`
// so mode crates contributing keymaps via `Mode::keymap()` can
// name their bindings' binding-mode without depending on
// `lattice-host`. Re-exported here so the existing
// `lattice_host::keymap::BindingMode` paths used by the
// dispatcher, the TUI input layer, and the GPUI peer keep
// resolving verbatim.
