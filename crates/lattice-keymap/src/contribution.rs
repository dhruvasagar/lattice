//! `Keymap` and `KeymapBinding` — a mode's declarative keymap contribution.
//!
//! K.3 (2026-06-07): moved from `lattice-mode::contributions` into
//! `lattice-keymap` so `KeymapLayer`, the trie, and future
//! `resolve_trace` can reference these types without a dep cycle.
//!
//! `lattice-mode::contributions` is retained as a re-export shim for
//! `Keymap`, `KeymapBinding`, `Subscription`, and `DecorationProvider`.
    ...
    fn bind_chord_captures_source_at_call_site() {
        ...
        match &km.bindings[0].source.kind {
            SourceKind::File { line, path } => {
                ...
                assert!(
                    path.to_string_lossy().ends_with("contribution.rs"),
                    "source path = {}",
                    path.display(),
                );
            }
