...
/// populated from `RenderState::lsp.progress`. `LspProgressMode`
/// reads it via `ctx.service::<LspProgressStatusData>()`.
pub struct LspProgressStatusData {
...
/// `lsp-progress-mode` — hand-written (not macro-generated) so
/// `status_line_items` can surface in-flight `$/progress` tokens
/// in the per-pane status line. Otherwise identical to the
/// marker sub-modes generated by `lsp_sub_mode!`.
...
impl Mode for LspProgressMode {
    ...
    /// MO.4.b: show the highest-priority in-flight LSP progress
    /// token. Picks the entry with the largest percentage (or any
    /// non-End entry when percentages are absent) across all
    /// servers — no per-buffer filtering yet (MVP; refinement
    /// adds server-id scoping once the App registers per-pane
    /// attached server ids).
    fn status_line_items(&self, ctx: &StatusLineCtx<'_>) -> Vec<StatusLineItem> {
        let Some(data) = ctx.service::<LspProgressStatusData>() else {
            return Vec::new();
        };
        let mut best: Option<&crate::events::LspProgressUpdate> = None;
        for ((_sid, _tok), update) in data.progress.iter() {
            if matches!(update.kind, crate::events::LspProgressKind::End) {
                continue;
            }
        ...
        vec![StatusLineItem { text, priority: 70 }]
    }
    fn on_activate(&self, _ctx: ModeContext) -> LifecycleFuture<'_, ()> {
        Box::pin(async { Ok(()) })
    }
