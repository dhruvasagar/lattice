...
//!
//! K.2.4.A.0.1 (2026-06-02): relocated from `lattice-host::keymap`
//! into `lattice-mode::keymap_entry` so mode crates that contribute
//! keymaps via `Mode::keymap()` can declare bindings through
//! [`keymap_entry!`] without depending on `lattice-host`. The host's
//! `lattice-host::keymap` is retained as a re-export shim, so
//! `:describe-key`, `:keymap`, and the TUI drift-test catalog walker
//! keep resolving verbatim.
//!
//! ## Construction
//!
...
/// own distinct line.
///
/// K.2.4.A.0.1: usable from any crate that depends on `lattice-mode`
/// (mode crates contributing keymaps via `Mode::keymap()`). The path
/// qualifier `$crate::keymap_entry::KeymapEntry::__new` resolves
/// inside `lattice-mode`; external callers still write
/// `lattice_mode::keymap_entry! { … }` or pick up the re-export at
/// `lattice_host::keymap_entry!`.
#[macro_export]
