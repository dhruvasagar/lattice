//! PH7.0 instantiation smoke bench.
//!
//! Two measurements: (1) instantiate a pre-compiled component — the
//! per-invocation cost the lazy-instantiation model (PH7.1) pays on a
...
//! cache (PH7.1) exists. Neither is a gated budget yet; the cold-start and
//! per-call ratchets land at PH7.1 / PH7.5. This bench exists so the surface
...
fn instantiate(c: &mut Criterion) {
    ...
    c.bench_function("plugin_instantiate_noop", |b| {
        b.iter(|| {
            let plugin = host
                .instantiate(black_box(&component))
                .expect("instantiates");
            black_box(plugin);
        });
    });
    ...
    c.bench_function("plugin_compile_instantiate_noop", |b| {
        b.iter(|| {
            ...
            let plugin = host.instantiate(&component).expect("instantiates");
            black_box(plugin);
        });
    });
}
