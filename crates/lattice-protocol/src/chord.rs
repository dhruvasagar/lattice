...
///
/// `Literal` matches a specific chord; `CharLiteral` matches any
/// single bare-char chord (no modifiers). The wildcard captures
/// the matched char so the resulting [`crate::ids::CommandId`]
/// invocation can receive it (mark / register / find-char
/// targets all flow through this).
///
/// Lives in `lattice-protocol` alongside [`KeyChord`] because
/// mode crates (which contribute keymap bindings via
/// `Mode::keymap()`) need to construct paths without depending
