...
impl App {
    ...
    pub(super) fn do_lsp_call_hierarchy_request(&mut self, outgoing: bool) {
        ...
        let label =
            word_under_cursor(&snapshot_for_label.buffer, self.editor.cursor).unwrap_or_default();
    ...
    pub fn maybe_request_document_highlight(&mut self) {
        ...
        let snapshot = self.editor.document.snapshot();
        let Some(position) = crate::app::app_to_lsp_position(&snapshot.buffer, self.editor.cursor)
        else {
            return;
        };
        ...
        match server_id {
            ...
            Some(id) => {
                ...
                for inst in seen {
                    self.editor
                        .lsp_logger
                        .set_instance_level(inst, Some(parsed));
                    applied += 1;
                }
                ...
                if applied == 0 {
                    ...
                    self.editor
                        .lsp_logger
                        .set_instance_level(synth, Some(parsed));
                    applied = 1;
                }
                self.set_message(
                    EchoLevel::Info,
