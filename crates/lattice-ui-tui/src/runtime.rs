...
fn main_loop(terminal: &mut Terminal<TermBackend>, mut app: App) -> Result<()> {
    ...
    // Tracks the active buffer + its line count across frames so a buffer
    // switch that SHRINKS the content can force a full repaint (see the
    // clear in the draw loop).
    let mut last_active_buffer: Option<(crate::buffers::BufferId, usize)> = None;
    ...
    while !app.render_state.load().lifecycle.should_quit {
        ...
        // Force a full repaint when a buffer switch SHRINKS the content. A
        // switch to a shorter buffer (picker live-preview flipping to a
        // smaller file, the binary "<no preview>" placeholder, or the
        // no-match restore to the empty origin) leaves rows the previous
        // buffer painted that the new (shorter) content can't cover; ratatui
        // leaves those stale in the live async run — the preview "bleeding"
        // (line-tail fragments surviving until a manual `<C-l>`). Clearing
        // makes the next draw repaint every cell, like RedrawScreen.
        //
        // Scoped to SHRINK only: switching among full-screen buffers (the
        // common preview-typing case) grows/keeps the line count, so it
