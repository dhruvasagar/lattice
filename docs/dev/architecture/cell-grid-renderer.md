...
| **S4.final.b** | ⛔ planned | `paint_cells(matrix_slice, scroll, viewport, window, theme)` function: iterates the active-pane matrix, emits `paint_quad` for `cell.bg != 0` and `paint_glyph(origin, glyph_id, fg)` per cell. Origin advances via `TextSystem::advance(font_id, size, ch)`. Wired into `EditorElement::paint` behind a runtime toggle; both `shape_line` and `paint_cells` paths coexist until S4.final.f. Cursor / overlays still painted from the prepaint ShapedLine bookkeeping. |
| **S4.final.c** | ⛔ planned | Hit-testing on the cell grid. Replaces `ShapedLine::closest_index_for_x` lookups with a cumulative-advance walk over `CellRow.cells`. Click + cursor x/col positioning land on the cell grid. Mouse-select / drag-select use the same walk. |
| **S4.final.d** | ⛔ planned | Modifier rendering on the paint path: UNDERLINE geometry via `paint_quad` (1-px line at the font's underline position); DIM via per-cell color attenuation (mirrors `cells_paint::dim_channel`); REVERSE applied at paint time. Visual parity with the TUI cells path and with the existing `cells_paint` converter. |
| **S4.final.e** | ⛔ planned | Emoji + font-fallback handling. Cells whose primary-font `GlyphId` is `0` (`.notdef`) fall back through `layout_line` to pick up the system's fallback font + `paint_emoji` for color glyphs. The `GlyphResolver` cache stores the resolved `(font_id, glyph_id, is_emoji)` triple so fallback is paid for once per codepoint, not per frame. |
| **S4.final.f** | ⛔ planned | Retire `shape_line` on the active-pane document body. `EditorElement`'s legacy `shape_row` / `build_line_with_inlays` / `LineRunBuilder` / `make_run_with_color` delete. The runtime toggle from S4.final.b retires; `paint_cells` becomes the only document-body path. `cells_paint::cell_row_to_text_runs` retires alongside it (its only caller was the S4.1 `shape_row_from_cells` closure, also retired). Status line / popup / picker / help / gutter / synthetic buffers keep their Shaped paths. |
