...
| **S3.b** | ⛔ planned | TUI helper `CellRow → Vec<Span<'static>>` honouring fg + modifier flags. Wired into `draw_buffer`'s per-line path for document buffers; falls back to RowPrepaint when matrix empty. Side-by-side validation tests. |
| **S3.c** | ⛔ planned | Retire RowPrepaint for code-class buffers once S3.b is solid. RowPrepaint stays for markdown/help/popups (rich rendering). Closes the dual-path window. |
