...
| **S4.1** | ⛔ planned | Body wiring foundational slice. `EditorElement` gains an optional `cell_matrix: Arc<CellMatrix>` field populated from `render_state.cells.matrix.load_full()`. `prepaint`'s body branches add a `shape_row_from_cells` closure (cells lookup via `CellMatrix::row_at_source_line(line_idx)` → `cell_row_to_text_runs` → `shape_line`); dispatch becomes a 3-way fallback `cells → prepaint → legacy`. Workspace stays green; existing legacy/prepaint paths handle every gap. Mirrors S3.c.0. |
| **S4.2** | ⛔ planned | Modifier propagation. Read BOLD / ITALIC bits from the cell into the `TextRun.font` weight/style; UNDERLINE + DIM + REVERSE land as per-cell renderer flags. Visual parity with the TUI converter. |
| **S4.3** | ⛔ planned | Retire `shape_row_from_prepaint` for active-pane document bodies once the cells path is validated. Highlights worker's `visible_rows` keeps publishing for inactive panes / non-document bodies. |
| **S4.final** | ⛔ planned | Glyph atlas + `paint_cells`. Replaces `shape_line` on the code-class hot path. Atlas keyed by `(codepoint, font_id, size_px)`; lazy raster on miss via GPUI's font system. Status line / popup / picker / help stay on the Shaped path. |
