...
Anchor: [design.md §5.6](design.md) (rendering layered architecture) and paramount goal #1 (sub-frame keystroke→glyph at 120 Hz, within the one-frame ceiling ≤ 8.3 ms).
...
| **S2.4.b** | ✅ done | Smallest-rebuild-set + downstream chunk shift. New `lattice_cells::EditDelta` carries `(start_line, lines_removed, lines_added, old_end_at_bol)` — the fourth field landed in S2.4.c below; see that row for why the first three alone are ambiguous. `CellRow::with_source_line` and `CellChunk::shifted_by` reuse the cell `Arc` payload (refcount bump only) when shifting downstream chunks. `Editor.last_edit_for_cells` tracks the per-publish-cycle single-edit delta (set in `publish_document_changed`, cleared on multi-edit / batch / undo / redo / non-single-edit mutation, `take()`n by `build_render_state`). `CellsRenderState.last_edit` carries it to the worker. `try_incremental_build` checks eligibility (single-text-edit step, other axes unchanged, mode + chunk_size compatible, line-count consistency for doc-switch guard) and either: in **whole-doc** mode rebuilds the only chunk; in **chunked** mode partitions published chunks into prefix-reuse (`Arc::clone`), rebuild zone (`build_chunk_rows` over `[rebuild_lo, rebuild_hi)`), and suffix-shift (`shifted_by(net_delta)`) regions. New `WorkerDecision::RecomputedIncremental` variant. 7 tests cover whole-doc-with-edit, chunked-prefix-reuse + suffix-shift with cell-Arc identity sharing, deletion shifts negative, eligibility failures (no last_edit / theme axis / line-count mismatch / mode change). |
| **S2.4.c** | ✅ done | Fix: table-align's last row stayed stale until `<C-l>`. `EditDelta`'s `lines_removed`/`lines_added` are a tree-sitter-shaped ROW delta (`old_end_position.line - start_line`), which can't distinguish "old range ends at BOL of a genuinely untouched line" from "old range ends partway into (or at EOL of) its own last line" — the shape table-mode's `rewrite()` and org's `replace_lines` build on EVERY invocation (range end = EOL of the last affected line, never BOL of the line after). `try_incremental_display_build`'s `suffix_lo` computation read the ambiguous case as "untouched," reused the pre-edit row for the table's last line verbatim, and stamped the `DisplayMatrix` version current — so the stale row was never revisited without a full rebuild (not "rare" or "self-healing," as a prior comment on that computation claimed; it reproduced on every align). Fix: `EditDelta` gains a fourth field, `old_end_at_bol: bool` (set in `dispatch.rs`'s `cells_edit_delta_from_applied` from `old_end_position.byte == 0`), and a new `EditDelta::suffix_start_line()` method centralises the three-way boundary decision (pure insert / clean line boundary / mid-line boundary) that `pre_edit_end_line()` alone can't make — `pre_edit_end_line()`'s doc comment no longer claims lines `>=` it are untouched. `#[derive(Default)]` on `EditDelta` defaults `old_end_at_bol` to `false` (conservative: one extra row rebuilt, never a wrongly-reused stale one), so the ~15 pre-existing construction sites that don't care about this axis opt out via `..Default::default()`. Both renderer peers (`lattice-ui-tui`, `lattice-ui-gpui`) are unaffected — they consume the already-decided `DisplayMatrix` rows read-only and carry no independent staleness logic. 6 new tests: 3 on `EditDelta::suffix_start_line()`'s three branches + the conservative default, 2 `cells_worker` regression tests asserting the produced `DisplayMatrix` row text (not buffer text) for the table-align whole-doc shape and a general interior-range mid-line-boundary replace with genuine untouched prefix/suffix lines, covering org's `replace_lines` shape too. |
