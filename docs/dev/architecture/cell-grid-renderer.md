...
| **S2.4** | 🔄 in progress | Chunked mode + incremental rebuild. Sub-sliced as S2.4.a (structural switch) and S2.4.b (smallest-rebuild-set + downstream chunk shift; needs edit-range plumbing). |
| **S2.4.a** | ✅ done | Chunked-mode structural switch. New `pick_chunk_size(viewport_height, line_count)` returns `WholeDoc` when `viewport_height == 0` or `line_count ≤ 4 × viewport_height`, otherwise `Chunked(next_pow2(2 × viewport_height))` (clamped to a 16-line floor). `build_matrix` orchestrates: a `ChunkInputs<'_>` borrow bundle is built once (syntax spans, inlay buckets, fold index, theme) and a small `build_chunk_rows(&inputs, start, end)` helper produces each chunk's rows; the orchestrator dispatches to `CellMatrix::whole_doc` or `CellMatrix::chunked`. Every chunk still carries the same publisher `MatrixVersion` — S2.4.b will swap reused chunks via per-chunk version comparison. 6 tests cover the policy table, `next_power_of_two` edges incl. `u32::MAX` saturation, small-doc whole-doc, large-doc chunk count / start_source_line / row content, fold elision across chunk boundaries, and viewport-shrink-promotes-to-chunked. |
| **S2.4.b** | ⛔ planned | Smallest-rebuild-set + downstream chunk shift. Threads `(start_line, lines_removed, lines_added)` edit deltas from the document into the cells substate. Worker compares per-chunk versions against publisher inputs; chunks whose covered range is unaffected reuse their `Arc<CellChunk>`; chunks past the edit shift `start_source_line += Δ` without rebuild. |
