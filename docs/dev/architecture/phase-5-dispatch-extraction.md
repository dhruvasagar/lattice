...
- **5.5.E.7** ✅ *(resurrected)*: full migration of the `apply_edit_blocking` / `handle_edits` cluster across 7 sub-slices.
  - **E.7.1** ✅ `shift_highlights_for_edit` + private `shift_spans_within_line` → `Editor`. **Audit revealed no signal needed**: the body only touches `editor.scroll` + `editor.visible_highlights` (both host-side fields per Bucket-A reclassification done in F.4.x). The original plan for `RendererSignal::EditsApplied(Vec<EditDelta>)` is retired; the renderer never sees the deltas because the host shifts the cache directly.
  - **E.7.2** ✅ `publish_document_changed` → `Editor`. App-side wrapper retained for 5 prod call sites until E.7.3 + E.7.7 close them; `App::shift_highlights_for_edit` wrapper deletes (zero remaining callers post-E.7.1).
  - **E.7.3** ✅ `apply_edit_blocking`, `apply_edit_batch_blocking`, `undo_blocking`, `redo_blocking`, private `apply_edit_to_oil` → `Editor` (`cac91be`). App-side wrappers collapse to delegates; `App::apply_edit_to_oil` deletes.
  - **E.7.4** ✅ `do_delete_line` → `Editor`; `Effect::DeleteCurrentLine` arm migrates to `Editor::handle_effect`.
  - **E.7.5** ✅ `do_substitute` → `Editor`; `Effect::Substitute` arm migrates. Pulls `fancy-regex` as a direct `lattice-host` dep (mirrors `lattice-core::search` usage). Stale `Edit` import drops from `app/search.rs`.
  - **E.7.6** ✅ *(planner-only)* `build_global_targets` → `Editor` (pattern validation + addressable-line scan + zero-match echo). **The body-replay loop stays App-side** until the `Effect` router fully migrates — a not-yet-migrated body effect would be a silent no-op via `Editor::handle_effect` alone, since `App::apply_effect` still owns the full router. Once G.x retires `App::apply_effect`, the loop joins the planner host-side.
  - **E.7.7** ✅ `handle_edits` chokepoint → `Editor`; `Effect::Edits(edits)` arm migrates to `Editor::handle_effect`. **App-side `handle_edits` + `publish_document_changed` wrappers delete entirely** (zero callers). Stale `AppliedEdit` imports drop from `app/dispatch.rs` + `app/lifecycle.rs`.
- **5.5.F.4**: `activate_buffer` + `activate_document` / `activate_file_tree` / `activate_help_in_pane` / `activate_oil` → `Editor`, alongside `snapshot_active_pane` / `snapshot_active_document` / `load_active_pane` / `activate_buffer_state`. Migrate `Effect::BufferNext` / `BufferPrev` / `BufferDelete` to `Editor::handle_effect`. Emits transient `RendererSignal::BufferActivated` for the App-side `activate_buffer_state` tail (mode/syntax re-resolve, visible-highlights cache clear) — deletes / transforms when F.5 lands mode lifecycle host-side. Sub-sliced F.4.1 (snapshot/load plumbing) ✅, F.4.2 (`activate_*` dispatch family with `bool` cross-window coordination) ✅, F.4.3 (`Effect::BufferNext` / `BufferPrev` + `RendererSignal::BufferActivated`) ✅, F.4.4 (`do_buffer_delete` + `lsp_close_buffer` + `Effect::BufferDelete` — the `lsp_close_buffer` audit landed clean: both `buffer_uris` and `lsp` already on `Editor`, no signal placeholder needed) ✅.
- **5.5.F.5** ✅: mode lifecycle — sub-sliced F.5.1 (pure-editor dep prep: `lsp_mode_enabled_for`, `recompute_active_completion_sources_for`, `path_for_buffer`) ✅; F.5.2 (`activate_mode_by_id` + `deactivate_mode_by_id` + `maybe_auto_activate_lsp_mode` + `toggle_mode_by_name`) ✅; F.5.3 (`activate_major_for_buffer_kind` + `drain_mode_lifecycle_events`) ✅; F.5.4 (`mirror_option_to_modes` → host; `RendererSignal::MirrorOptionToModes` retired — cascade runs synchronously inside `apply_option_cascade`) ✅; F.5.5 (`activate_buffer_state` → host; `RendererSignal::BufferActivated` retired — Bucket-A cache clear is plain Editor-field writes, no replacement signal needed) ✅. Lifecycle methods that emit signals return `Vec<RendererSignal>`; App-side wrappers fan via `handle_renderer_signal`.
- **5.5.F.6** ✅: `:describe-mode` / `:list-modes` / `:customize` content builders → `Editor`; Effect arms route through `DisplayBuffer` pipe. Five host builders shipped (`build_list_modes_content`, `build_describe_mode_content`, `build_customize_picker_content`, `build_customize_group_content`, `build_customize_mode_content`) + the shared `append_customize_row` helper. App-side `do_describe_mode` and `do_customize` retained as thin link-follow wrappers (`HelpLinkTarget::Mode` / `HelpLinkTarget::Customize` follow-handlers); the rest deleted entirely.
- **5.5.F.7** ✅: `:list-diagnostics` → `Editor`. **Different signal shape from F.1–F.6** — this Effect produces a picker, not a help buffer; the picker is a renderer-neutral `Editor` field (`self.picker`) that the renderer reads each frame, so no `RendererSignal` is required. `Effect::ListDiagnostics` arm dispatches as a direct `editor.do_list_diagnostics()` call. App-side wrapper retained as a 1-line `#[allow(dead_code)]` delegate for three test callers.
- **5.5.G**: collapse `App::apply` — sub-sliced because the body's still ~420 LoC after F.7 (the original "trivial collapse" framing assumed E covered the LSP / completion / motion clusters, but Bucket-A audits in F kept most of those App-side). Each sub-slice migrates a cluster of pure-editor `do_*` helpers + their apply arms; App's match drops them into the grouped no-op alongside the 5.5.C set.
  - **G.1** ✅ pure-editor fold / macro / snippet arms (11 arms). Eight fold arms (`OpenFoldAtCursor`, `CloseFoldAtCursor`, `ToggleFoldAtCursor`, `OpenAllFolds`, `CloseAllFolds`, `DeleteFoldAtCursor`, `GotoNextFold`, `GotoPrevFold`), two macro arms (`StartMacroRecord`, `StopMacroRecord`), and `SnippetLeave` move to `Editor::dispatch`. Helpers `do_set_fold_state_at_cursor`, `do_set_all_folds`, `do_goto_fold`, `do_delete_fold_at_cursor`, `do_start_macro_record`, `do_stop_macro_record` migrate to `Editor`; private selection-rule helpers `innermost_fold_idx`, `fold_to_close_at`, `outermost_fold_idx` co-move. App-side helpers delete entirely (zero callers).
  - **G.2** ✅ pure-editor visual + mark arms (4 arms). `EnterVisual`, `ExitVisual`, `ReselectLastVisual`, `SetMark`. `do_enter_visual` / `do_reselect_visual` migrate to `Editor`; `do_exit_visual` retained as a 1-line delegate because four App helpers still call it (`run_oil_invocation`, post-operator-on-selection path, `do_create_fold_from_visual`, LSP rename helper).
  - **G.3** ✅ pure-editor edit-cluster arms (10 arms). `Undo`, `Redo`, `JoinLines`, `ToggleCaseAtCursor`, `EnterAppend`, `OpenLineBelow`, `OpenLineAbove`, `OverwriteChar`, `ReplaceUndoLast`, `DeleteCharBackward`. Eight helpers + private `previous_position` migrate to `Editor`. App-side helpers all delete (zero callers); App's `redo_blocking` wrapper picks up `#[allow(dead_code)]`.
  - **G.4** ✅ pure-editor scroll / viewport / bracket / redraw arms (8 arms). `JumpViewport`, `ScrollCursorTo`, `PageDown`, `PageUp`, `ScrollLineUp`, `ScrollLineDown`, `MatchBracket`, `RedrawScreen`. Six helpers + `auto_open_folds_at_cursor` migrate to `Editor`; the latter is retained App-side as a 1-line delegate because 7 ui-tui call sites still invoke it (`search.rs`, `motions.rs`, `dispatch.rs`).
  - **G.5** ✅ pure-editor pane-navigation arms (6 arms). `SplitPaneHorizontal`, `SplitPaneVertical`, `ClosePane`, `NavigatePane`, `NextPane`, `PrevPane`. Five helpers (`do_split_pane`, `do_close_pane`, `do_navigate_pane`, `activate_pane`, `buffer_area_rect`) migrate to `Editor`; `do_close_pane` + `activate_pane` retained as App-side delegates because `do_quit` and other App helpers still call them. Note: G.5 originally targeted completion arms but reverted — `do_completion_next/prev` call `refresh_docs_popup_for_selection` which depends on the App-side `do_completion_resolve_focused` (LSP fetch); rescoping to panes kept the slice contained.
  - **G.6** ✅ pure-editor mark-history walk (2 arms). `WalkMarkHistoryBack`, `WalkMarkHistoryForward`. `do_mark_history` + `do_walk_history` (the ~70 LoC unified ring walker with cross-buffer landing for Document/Help/FileTree/Oil entries) migrate to `Editor`. App-side `do_walk_history` retained as a 1-line delegate because `do_jump_history` (`<C-o>`/`<C-i>`) still calls it; jump-history blocked on `pop_popup_back` migrating host-side (depends on `seed_help_metadata_locals`).
  - **G.7+** deferred. Remaining clusters in `App::apply`:
    - **Insert / EnterMode cluster** — `do_insert_text` triggers LSP signature-help and on-type-formatting autopilot (`do_lsp_signature_help_request`, `do_lsp_on_type_formatting_request`); `enter_mode` calls `replicate_block_insert`. Migration blocked on the LSP request senders moving host-side.
    - **Completion cluster** (`CompletionTrigger`, `CompletionNext`, `CompletionPrev`, `CompletionAccept`, `CompletionCancel`, `CompletionToggleDocs`, `CompletionDocsScroll*`, `CompletionFilter*`, `CompletionAcceptThenInsert`) — depends on `refresh_docs_popup_for_selection` → `do_completion_resolve_focused` (App, LSP-coupled), `refilter_insert_completion` (App), and the docs-body helpers.
    - **Picker cluster** (`PickerAppend`, `PickerBackspace`, `PickerSelectNext`, `PickerSelectPrev`, `PickerAccept`, `PickerDismiss`) — accept/dismiss have heavy App-side coupling (file open, LSP locations, SMR queue, preview restore via `activate_buffer`); the simple Append/Backspace/Select* call `bump_live_picker_debounce` + `preview_picker_selection`.
    - **Help / Oil / FileTree follow + dismiss** — `do_help_follow_link`, `do_oil_follow`, `do_oil_navigate_up`, `do_file_tree_follow`, `dismiss_file_tree`. Coupled to renderer surfaces and link/path resolution helpers that haven't moved.
    - **Search cluster** (`SearchAppend`, `SearchBackspace`, `SearchSubmit`, `SearchCancel`, `SearchNext`, `SearchPrevious`, `SearchWordUnderCursor`, `FindRepeat`) — bodies use App-side `preview_search`, `submit_search`, `cancel_search`, `repeat_search`, `compile_search_pattern`, `step_byte`, `is_word_char_byte`. Mostly editor-only but a single batch migration would touch ~200 LoC of search machinery.
    - **CommandLine cluster** (`EnterCommandLine`, `CommandLineAppend`, `CommandLineBackspace`, `CommandLineSubmit`, `CommandLineClear`, `CommandLineDeleteWordBackward`, `CommandLineDescribeUnderCursor`, `CommandLineAppendChord`, `CommandLineCompleteOrAdvance`, `CommandLineCompletePrev`, `CommandLineAcceptCompletion`, `CommandLineHistoryPrev`/`Next`) — entangled with completion popup state + substitute preview + the cmdline-describe handlers.
    - **LSP request arms** (~12 arms) — each routes through `do_lsp_*` helpers.
    - **Snippet expand/next/prev**, **TagStackPop**, **PlayMacro/PlayLastMacro**, **JumpHistoryBack/Forward**, **JumpToMark{Line,Exact}**, **PasteAfter/Before/Text**, **RepeatLastChange**, **EnterBlockVisualInsert/Append**, **Invoke**, **Insert**, **EnterMode**, **CreateFoldFromVisual**, **ToggleFoldEnable**, **CloseHover**, **FollowLink** etc. — each has 1+ App-side dependencies.
  
  G.1–G.6 cumulative: 41 arms migrated; App::apply body shrunk from ~420 LoC to ~280 LoC. Tests: 1419 ui-tui + 185 host + 180 lsp = 1784 green at every sub-slice boundary. **G.final (true `App::apply` collapse) requires the deferred clusters' App-side helpers to migrate first** — most non-trivially LSP request senders + completion-resolve + popup back-stack. Each is a slice of comparable size to E.7 / F.5. A staged G.7..G.N continuation lands those clusters one by one; the final collapse becomes mechanical once App-side helpers all delegate.

  - **G.7** ✅ tag-stack + mark-jump + popup-back-stack + jump-history (5 arms: `TagStackPop`, `JumpToMarkLine`, `JumpToMarkExact`, `JumpHistoryBack`, `JumpHistoryForward`). The deferral predicted in G.6 closed sooner than expected: `seed_help_metadata_locals` is pure-editor (its mode-key types `HelpLinks`/`HelpAnchors`/`HelpHighlights` already live in `lattice_host::modes`), which unblocks `pop_popup_back` → `do_jump_history`. Helpers migrated: `seed_help_metadata_locals`, `pop_popup_back`, `do_tag_stack_pop`, `do_jump_mark`, `do_jump_history`. App-side `seed_help_metadata_locals` / `pop_popup_back` / `do_jump_mark` retained as 1-line delegates (multiple App callers across lifecycle/popup/test_helpers/motions/picker); `do_tag_stack_pop` and `do_jump_history` deleted.
  - **G.8** ✅ snippet placeholder navigation (2 arms: `SnippetNextPlaceholder`, `SnippetPrevPlaceholder`). `do_snippet_expand_at_cursor` stays App-side pending `active_language_id` + `is_word_char_byte` migration. Helpers `do_snippet_next_placeholder`, `do_snippet_prev_placeholder`, and private `move_cursor_to_snippet_group` migrate to `Editor`. App-side wrappers retained as 1-line delegates because direct test callers in `app/completion.rs` invoke them.
  - **G.9** ✅ paste cluster (3 arms: `PasteAfter`, `PasteBefore`, `PasteText`). All four helpers — `do_paste_text`, `do_paste`, `do_paste_blockwise`, `read_register` — migrate to `Editor`. App-side `do_paste` / `do_paste_text` kept as 1-line delegates because `app/picker.rs` (paste-picker accept) and the bracketed-paste runtime hook still invoke them. `read_register` and `do_paste_blockwise` deleted (zero remaining callers). Now-test-only imports `YankKind` / `UnnamedRegister` gate behind `#[cfg(test)]`.
  - **G.10** ✅ search-state cluster (7 arms: `SearchAppend`, `SearchBackspace`, `SearchSubmit`, `SearchCancel`, `SearchNext`, `SearchPrevious`, `SearchWordUnderCursor`). `Action::FindRepeat` stays App-side (calls `run_invocation`); the substitute-preview path (`refresh_substitute_preview` + `collect_substitute_matches_for_line` + `do_substitute` delegate) stays App-side because it's driven from the cmdline executor and uses App-side `compile_search_pattern`. Helpers `preview_search`, `submit_search`, `cancel_search`, `repeat_search`, `do_search_word_under_cursor` migrate to `Editor`. Free fns `compile_search_pattern`, `step_byte`, `is_word_char_byte` duplicated host-side (App copies stay for the substitute path). App-side helpers all deleted.

  G.1–G.10 cumulative: **58 arms migrated; App::apply body ~420 LoC → ~235 LoC**. Tests: 1419 ui-tui + 185 host + 180 lsp = 1784 green at every boundary.

  - **G.11** ✅ picker append/select + close-hover (5 arms: `PickerAppend`, `PickerBackspace`, `PickerSelectNext`, `PickerSelectPrev`, `CloseHover`). `PickerAccept` / `PickerDismiss` stay App-side (file-open / SMR queue / preview-restore coupling). Helpers `bump_live_picker_debounce` and `preview_picker_selection` migrate to `Editor`; the latter returns `Vec<RendererSignal>` so the picker arms fan signals from the inner `activate_buffer_state` tail through `DispatchOutcome` (same pattern as F.5.2's mode-lifecycle wrappers). `CloseHover` inlines as a direct `editor.dismiss_popup()` call.
  - **G.12** ✅ help-dismiss (1 arm: `HelpDismiss`). Dispatches on `active_buffer` to either `dismiss_popup` (already host-side) or `dismiss_file_tree` (now migrated; returns `Vec<RendererSignal>` because the successor `activate_buffer_state` tail can fan mode-lifecycle signals like `lsp-mode` attach). App-side `dismiss_file_tree` retained as a 1-line delegate that fans the returned signals — `Effect::CloseFileTree` apply_effect arm continues to compile. `FollowLink` (dispatches to `do_help_follow_link`/`do_oil_follow`/`do_file_tree_follow`), `OilNavigateUp` (uses `oil_dir_for`/`set_oil_dir`/`file_tree_entries_for`), and the Picker accept/dismiss family remain in the deferred G.x backlog.

  G.1–G.12 cumulative: **64 arms migrated; App::apply body ~420 LoC → ~205 LoC**. Tests: 1419 ui-tui + 185 host + 180 lsp = 1784 green at every boundary.

  - **G.13** ✅ pure-editor command-line arms (3 arms: `EnterCommandLine`, `CommandLineHistoryPrev`, `CommandLineHistoryNext`). `EnterCommandLine` inlines the modal+state pivot directly into the host match (uses `editor.dismiss_popup()` which has been host-side since 5.5.D). The history-step arms route through new `Editor::do_command_history_step`, a verbatim relocation of the snapshot/restore-pending walker.
  - **G.14** ✅ completion cancel cluster + foldenable toggle (5 arms: `CompletionCancel`, `CompletionCancelAndExitInsert`, `CompletionDocsScrollDown`, `CompletionDocsScrollUp`, `ToggleFoldEnable`). Helpers `do_completion_cancel`, `do_completion_docs_scroll_{down,up}` migrate to `Editor`. App-side `do_completion_cancel` retained as `#[allow(dead_code)]` delegate (still called by `<Esc>` flush / snippet abort / `run_invocation` exit). `ToggleFoldEnable` inlines: read `option_cache.foldenable`, flip via `config.set_typed`, drain cascade via `editor.drain_option_changes()` and fan signals into `_out.renderer_signals`.
  - **G.15** ✅ cmdline-completion popup nav (2 arms: `CommandLineCompletePrev`, `CommandLineAcceptCompletion`). Helpers `do_command_line_complete_prev`, `do_command_line_accept_completion` migrate to `Editor` -- both pure-editor (only `completion_state` + `command_line` deps). `CommandLineCompleteOrAdvance` stays App-side; its open-popup path calls `open_completion_popup` -> `compute_completion_state` which uses `excommand::aliases()` (renderer-side). Migrating that needs the alias rewriter to land host-side first.
  - **G.16** ✅ `zf` Visual-fold creation (1 arm: `CreateFoldFromVisual`). `do_create_fold_from_visual` migrates verbatim -- all deps (`modal`, `document.selections()`, `folds`, `cursor`, `do_exit_visual`) were already host-side.
  - **G.17** ✅ modal pivot + blockwise-Visual I/A (3 arms: `EnterMode`, `EnterBlockVisualInsert`, `EnterBlockVisualAppend`). Helpers `enter_mode`, `do_enter_block_visual_insert`, `replicate_block_insert` co-migrate to `Editor`. `enter_mode` retains its full lifecycle: Replace-history reset, Insert-replay capture/promote, `pending_block_insert` commit via `replicate_block_insert`, Insert->Normal cursor pull-back, `ModalModeChanged` event publication. App-side `enter_mode` retained as delegate -- still called by `RepeatLastChange` apply arm, `Effect::EnterMode` apply_effect arm, and several `do_*` helpers in edit/motions/lsp modules pending their own migration. `do_enter_block_visual_insert` retained as `#[allow(dead_code)]` delegate for symmetry. `replicate_block_insert` deleted (zero remaining App callers).

  G.1–G.17 cumulative: **78 arms migrated; App::apply body ~420 LoC → ~165 LoC**. Tests: 1419 ui-tui + 185 host + 180 lsp = 1784 green at every boundary.

  **Remaining deferred G.x clusters (each a sized slice in its own right):**
  - **Insert / do_insert_text**: LSP-trigger autopilot (signature_help + on_type_formatting) blocks pure-editor extraction.
  - **Invoke / run_invocation**: recursion through document/oil/help/file_tree dispatch + LSP rename + run_help_invocation. Top-level command pipeline migration.
  - **CommandLine{Append,Backspace,Clear,DeleteWordBackward,Submit,DescribeUnderCursor,AppendChord,CompleteOrAdvance}**: blocked on App-side `refresh_completion_popup` / `refresh_substitute_preview` / `execute_ex_line` / `try_resolve_missing_arg_prompt` / `do_describe_command` / `open_completion_popup` / `compute_completion_state` (alias-rewriter).
  - **Lsp{Hover,Definition,Declaration,TypeDefinition,Implementation,References,FollowLinkAtCursor,SignatureHelp,Completion,DocumentSymbol,WorkspaceSymbol}Request**: each routes through App-side `do_lsp_*` request senders. Whole-cluster migration.
  - **Completion{Trigger,Next,Prev,Accept,ToggleDocs,FilterToSource,FilterClear,AcceptThenInsert}**: blocked on `refresh_docs_popup_for_selection` (LSP `completionItem/resolve`) and the LSP request fan.
  - **Picker{Accept,Dismiss}**: trait-driven accept path + SMR queue finalize + preview-restore + `prepare_pane_for_picker_result` + per-routing arms.
  - **SnippetExpand / do_snippet_expand_at_cursor**: depends on `active_language_id` + `expand_snippet` (with `snippet_variable_context`).
  - **PlayMacro / PlayLastMacro**: body recurses through `self.apply`; replaying Insert/Invoke through `Editor::dispatch` silently no-ops today (those arms still fall through host's `_ => {}`). Migrates as a sweep once Insert + Invoke land host-side.
  - **RepeatLastChange**: recurses through `run_invocation` + `do_insert_text` + `enter_mode`. Migrates with Invoke.
  - **FollowLink**: dispatches by buffer kind to `do_help_follow_link` / `do_oil_follow` / `do_file_tree_follow` -- each App-side.
...
- **5.5.H** ✅ *(first sweep — 2026-05-17)* vestigial-delegate cleanup. Audit caught 22 `#[allow(dead_code)]` markers + 6 unflagged unused symbols. Classification:
  - **Deleted (zero callers anywhere)**: `App::do_substitute`, `App::do_paste_text`, `App::do_enter_block_visual_insert`, `App::do_delete_line`, `App::do_split_pane`, `App::do_navigate_pane`, `App::activate_pane`, `App::buffer_area_rect`, `App::seed_active_document_locals`, `App::do_walk_history`, `App::pop_popup_back`, `App::move_cursor_to_snippet_group`. Free fns: `step_byte` + `previous_position` (search.rs / app.rs).
  - **Re-marked `#[allow(dead_code)]` (test-only callers)**: `App::undo_blocking` (test modules in `app/edit.rs`, `app/motions.rs` call directly for tighter assertions than `Action::Undo` permits), `App::bump_live_picker_debounce` (two test sites in `app/picker.rs` poke debounce directly for time-driven assertions).
  - **Kept untouched (genuine prod callers)**: `App::do_paste` (picker.rs paste-picker accept), `App::do_describe_mode` + `App::do_customize` (HelpLinkTarget follow-handlers), `App::do_close_pane` (App::do_quit), `App::document_last_parsed_text_version_for` + `App::document_last_synced_syntax_version_for` (highlights.rs prod path), plus test-driven delegates (`do_set`, `do_describe_events`, `do_describe_event`, `do_list_diagnostics`, `document_folds_for`, `set_popup_placement`, `do_completion_cancel`).
  - **Out of scope (predates G.x)**: `lattice-mode::BufferLocals::{get_mut,remove}` + `BufferLocal::{as_any_mut,into_any}` — typed-locals downcasting API surface from Phase 5.B, not dispatch-extraction debris.
  - **Orphan imports cleaned**: `PaneDirection`/`SplitOrientation` (lifecycle.rs), `SearchDirection`/`previous_position`/`line_byte_len` (search.rs), `PositionEntry` (motions.rs).

  Subsequent H sweeps fire opportunistically as each subsystem-hosting phase removes its remaining App-side callers; H.1 retires the second-wave delegates, H.2 retires the third-wave, etc.
...
- **Visible-highlights cache (`refresh_highlights` / `refresh_pane_highlights`)** — stays in ui-tui; render-coupled by design (per-frame paint cache keyed by viewport row). **Update (E.7.1):** `shift_highlights_for_edit` itself moved to `Editor` after the Bucket-A audit confirmed it only touches `editor.scroll` + `editor.visible_highlights` (both host-side fields). The original plan to surface it via `RendererSignal::EditsApplied` is retired — no signal needed.
...
The original framing ("E.7 is gated on the highlights cache") was correct *before* the signal-pipe pattern proved out in F.1. The post-F.3 plan reframed E.7 around an `EditsApplied(delta)` signal — but the E.7.1 audit went one step further and confirmed `shift_highlights_for_edit` itself is pure-`Editor` (the cache fields are host-side). E.7 therefore ships with **zero new `RendererSignal` variants**: the cache shift runs synchronously inside `publish_document_changed` before the renderer's next paint reads `visible_highlights`. Per-frame caches (`refresh_highlights`, `refresh_pane_highlights`) stay renderer-side as before.
