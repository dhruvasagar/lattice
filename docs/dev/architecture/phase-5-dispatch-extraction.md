...
**5.5.E — Move helpers, batch 2: ex-command effect handlers.** The big `apply_effect` table moves to `Editor::handle_effect`. Most `do_*` functions move with it. The LSP request senders, file ops, picker openers, format requests, snippet expansion — all of `app/lsp.rs`, most of `app/cmdline.rs`. (~3,000 LoC moved across multiple sub-slices; biggest cluster of the slice.)
  ...
  - 5.5.E.6 migrates the option-cascade infrastructure to host — `rebuild_option_cache`, `recompute_options_for_buffer`, `apply_option_cascade`, `drain_option_changes`, `mirror_option_to_modes`, `do_set`. Splits the ui.* arm: host calls `editor.sync_host_theme_from_config()` (a new method that reads typed options + writes `editor.host_theme`; the typed-option struct names may need to migrate from `lattice-ui-tui::tui_options` to host so the function is fully renderer-neutral) and emits `RendererSignal::ThemeChanged`. The ui.nerd_fonts secondary effect (`set_file_tree_nerd_fonts` rope refresh) either co-migrates or emits its own signal. The messages.filter branch (`lattice_runtime::reload_messages_filter`) is renderer-neutral and can run host-side directly. After E.6 lands, `Effect::SetOption` collapses into the grouped no-op in App's `apply_effect`. Subsequent E.* slices: the `apply_edit_blocking` / `handle_edits` cluster (`Effect::Edits`, `Effect::DeleteCurrentLine`, `Effect::Substitute`, `Effect::Global`) is gated on the render-coupled `shift_highlights_for_edit` cache — that's the visible-highlights slice the 5.5 design doc explicitly flags as out of scope, so this cluster may need to wait until post-5.5 work touches the highlights cache. In parallel, `display_buffer` host-side (a 5.5.F-shaped move) unlocks `Effect::ListBuffers` / `Effect::DescribeBuffer` / the describe-* family; `activate_buffer` migration unlocks `Effect::BufferNext` / `BufferPrev` / `BufferDelete`. Then come the LSP request senders, snippet ops, and finally `Effect::Many` (recursion through host) once enough inner arms have migrated.
