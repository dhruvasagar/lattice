...
Each slice ships green-on-merge with the four artifacts
CLAUDE.md mandates: design fragment (this doc, updated as needed),
tests, bench (where load-bearing), graceful error handling.
...
| **T-rich-1** (optional) | Text objects + advanced motions | Verify `iw`, `aw`, `i"`, `a"`, `i(`, `a(`, `%`, sentence / paragraph motions, `f`/`F`/`t`/`T` work against SyntheticDoc. Mostly verification + tests; the central implementations already exist. |
