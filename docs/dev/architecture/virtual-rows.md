...
- ✅ **D.0a** (2026-05-28, this slice) — Pure data layer in
...
- 🗒 **D.0a.1** — `virtual_rows_worker` on `lattice-host`.
  Sibling tokio task to `cells_worker`; wakes on Notify
  when a registered provider notifies mutation; calls
  `collect()` on each provider; merges; publishes
  `VirtualRowMatrix` via `ArcSwap`. New field on
  `RenderState`. Lands before D.3 or M.2 (whichever
  produces the first production consumer).
