...
> **Delivered as WT.3 + WT.4.** This section read as a proposal long after it
> was built, and that cost real advice: a reader (2026-09-22) planning a
> post-release ABI break took the old "Today" column at face value, concluded
> every user's `init.rs` would silently break, and recommended building
> machinery that already existed. The prose below is the reasoning, which
> still holds; the table now states what ships. Verify against source —
> `crates/lattice-plugin-loader/src/build.rs` is the implementation, and
> `Stamp` is the type.

**An ABI fingerprint is recorded at build time and compared at load.**
`.build-stamp` records what the artifact was built *from* **and what it was
built *against***: `lattice_wit::ABI_FINGERPRINT`, a hash over the whole `wit/`
package generated by `lattice-wit`'s build script. Nothing bumps it by hand —
editing any `.wit` file moves it.

| State | Behaviour |
|---|---|
| stamp matches on both axes | load the cached artifact; no toolchain invoked |
| source changed | rebuild |
| **ABI fingerprint differs** | **rebuild from source** (`build_plugin`, via `Stamp::matches`) |
| ABI differs, no rebuild allowed (pinned) | load anyway, `warn!` naming both fingerprints and pointing at `:plugin-unpin` |
| ABI differs, no source to rebuild from | fails to instantiate — surfaced, see below |

`build_plugin` also calls `refresh_wit_package(source_dir)` before building, so
a plugin that rebuilds picks up the new package without anyone copying files.

The consequence worth stating for planning purposes: **a plugin with source on
the machine heals itself.** `init.rs`, git sources and local directories all
rebuild on the next boot and the user sees nothing but one build's delay. Only
a pinned artifact or a prebuilt `.wasm` with no source is exposed, and both are
loud.

The pinned row was drafted as *refuse*, and refusing is wrong. The fingerprint
...
**And the failure surfaces (WT.4).** A plugin that fails to instantiate is a
...
than a glance. The arm that *was* the silent failure is named as such in
`install.rs`.

**And there is a repair command.** `lattice --wit-sync [DIR]` rewrites a plugin
source's `wit/` from this editor's package and exits without opening the editor
— the manual answer for a source tree whose copy has drifted, and the thing to
point a plugin author at when their component stops loading after an upgrade.
