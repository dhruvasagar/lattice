...
Nineteen slices shipped (A.4, F, A.3, C + follow-up, A.1, A.2a, B.1, D.1, E.1, E.2.α, A.2b.1, A.2b.2, A.2b.2b, A.2b.3, B.2.a, B.2.b, B.2.c, B.4.a, B.4.b). The plan's `ensure_us` and `highlights_us` UI-thread costs have been attacked: ensure work is gated and fold geometry is O(log); the worker now publishes pre-painted rows via `Arc<[T]>` with inlays already woven in so the GPUI fast path consumes `VisibleRows` unconditionally; the TUI peer reads the same `visible_rows` cell via `source_spans_from_runs`; the worker also pre-buckets the three static overlay layers (doc_highlight / all_matches / substitute) per row, so both renderer peers' active-pane prepaint paths read pre-computed quads instead of walking N_overlay × V_row intersections each frame. The active-pane overlay weave is now off the UI thread on both peers; high-N hlsearch (1000-match scale) no longer carries the O(N × V) per-frame tail-risk it had at A.2b.3. Editor publish itself is now identity-preserving on the seven highest-allocation sub-states (`panes` / `modes` / `buffer_locals` / `buffers` / `tabs` / inner `pane_highlights` / inner `lsp.progress`) so a no-op publish on a realistic fixture (3-pane tree, 20 LSP-attached buffers, 4 tabs, 6 in-flight `$/progress`) drops to ~52 % of the unmemoised cost.

**Plan closed.** E.2 (element-tree reuse) was the last open slice; the E.2.α investigation closed it without shipping further sub-slices because each candidate cost surface either fires only when an overlay is open (E.2.a/b/c — defer to a `profile-frames` trace on overlay-open sessions if one ever shows hot) or saves sub-µs in steady state (E.2.d / E.2.e). Future perf work runs through new slices grounded in fresh measurements — the prepaint bench (`editor_element_frame`) and publish bench (`dispatch_publish`) are the two CI-gated surfaces; the paint phase and overlay-open frames remain `profile-frames`-only until gpui exposes a headless test context.
...
| B.4.a — Identity-preserving Arc publish (5 subs) | done | `029271e` | `Versioned<T>` newtype + `PublishCache` on Editor. Cached: `panes`, `modes`, `buffer_locals` (full Arc); inner `syntax.pane_highlights`, inner `lsp.progress`. Steady-state publish 3.20 µs vs 6.05 µs unmemoised = **−47 %** on the B.4.a fixture. |
| B.4.b — Cache `buffers` + `tabs` (+ `BufferRegistry::version()`) | done | _commit pending_ | `BufferRegistry` gets an internal `AtomicU64` (bumped from every mutating method); `buffer_uris` + `tabs` wrapped in `Versioned`; `buffers` cached keyed on `buffer_uris.version()` (registry is shared `Arc<Mutex<>>` — same handle inside cached Arc sees current state), `tabs` cached on composite of (`tabs.version()`, `active_tab`, `pane_tree.version()`, `buffers.version()`). Realistic fixture (20 buffer_uris, 4 tabs): steady-state 3.23 µs vs 6.72 µs unmemoised = **−52 %**. Numbers below. |
| D.* — rayon / SmallVec / bump-alloc | dropped | —       | Bench review (below) didn't justify the impl cost.                 |
| E.2 — Element-tree reuse          | dropped | —         | E.2.α investigation found no bench-justified work: notify cadence is input-driven (no over-fire), conditional overlay-block construction is already correct, and the four sub-slices (E.2.a/b/c popup/picker/completion overlay shared elements; E.2.d tabline SharedString reuse; E.2.e render_state.load* consolidation) are either popup-open-only or sub-µs. See slice details. |
...
### Captured baselines (2026-05-25, post-B.4.b)

Bench: `dispatch_publish`. Reproduce: `cargo bench -q -p lattice-host --bench dispatch_publish`. Numbers are criterion median estimates. **Fixture grew at B.4.b** to include 20 LSP-attached `buffer_uris` + 4 tabs (alongside the earlier 3-pane tree, 20 buffers' worth of `active_modes` + `buffer_locals`, 3 panes × 60 spans `pane_highlights`, 6 in-flight `lsp_progress` items). The bigger fixture is why `unmemoised` climbed from 6.05 µs (B.4.a fixture) to 6.72 µs — that 0.67 µs of extra work is exactly what B.4.b now caches away. Same machine as the previous baselines.
...
- **B.4 saves ~52 % on no-op publishes (post-B.4.b) with zero net overhead on a fully-invalidated publish.** `unmemoised` (cache cleared between iterations, no per-iter mutation work) is the cleanest stand-in for pre-B.4 cost: every cached slot misses and rebuilds, just like the unconditional `Arc::new` path before B.4. The cache lookup machinery (`Mutex<PublishCache>::lock`, seven `Versioned::version()` / `BufferRegistry::version()` reads + one composite-hash, seven cache-slot compares, seven `Arc::clone`s when hits, one closure call + `Arc::new` per miss) doesn't show up — it's drowned by the rebuild cost it would normally avoid.
- **Steady-state stayed flat as the fixture grew at B.4.b.** Same 3.2 µs steady_state under both fixtures because the B.4.b caches absorbed the extra work the bigger fixture added. The `unmemoised` row climbing from 6.05 µs to 6.72 µs across the fixture change quantifies the B.4.b savings (~0.67 µs) on top of B.4.a's ~3 µs.
- **Steady-state floor (3.2 µs) is what's still rebuilt every tick:** `active_document` (cursor / scroll change every keystroke), the small Copy-or-Arc-clone sub-states (`lifecycle`, `theme`, `messages`, `modeline`, `options`, `diagnostics`, `translator`), the outer `lsp` / `syntax` sub-states (their other fields churn per-frame), and the picker / completion / popup Option-when-None slots.
- **Targeted cache invalidation works.** `mutated_modes` (one mode toggle per iteration) costs ~+14 % over steady_state because only the `modes` sub-state rebuilds; the other six cache hits stay. This is the design promise — a Mode activation doesn't force the renderer to drop any unrelated sub-state's Arc identity.
...
### B.4.b — Cache `buffers` + `tabs` (+ `BufferRegistry::version()`) [_commit pending_]
- `BufferRegistry` gets an `Arc<AtomicU64>` version counter alongside `inner: Arc<Mutex<...>>`. Every mutating method (`insert`, `remove`, `set_flags`, `set_name`, all six `with_*_mut` accessors) bumps it. `Versioned<T>`'s `DerefMut` autobump can't fire here because the registry exposes `&self` mutators (interior mutability) — so the bumps are explicit, scoped to the mutating method bodies.
- `buffer_uris` (`HashMap<BufferId, Uri>`) wrapped in `Versioned<HashMap<...>>` — autobump via `DerefMut` on the few `.insert/.remove` sites.
- `tabs` (`Vec<TabSlot>`) wrapped in `Versioned<Vec<TabSlot>>` — autobump on `.push/.remove/.insert` mutations.
- `PublishCache` gains two new slots: `buffers: Option<(u64, Arc<BuffersRenderState>)>` keyed on `buffer_uris.version()` (the inner `registry` field uses `Arc<Mutex>` interior mutability so the cached Arc's registry handle sees current state via the shared mutex even when the cache hits — no registry-version dependency needed for this slot's invalidation), and `tabs: Option<(u64, Arc<TabsRenderState>)>` keyed on a `DefaultHasher` fold of (`tabs.version()`, `active_tab as u64`, `pane_tree.version()`, `buffers.version()`).
- New test `buffers_substate_survives_registry_only_mutation_but_tabs_invalidates` pins the divergent invalidation behaviour: a `buffers.insert` doesn't change `buffer_uris.version()` so the `buffers` Arc survives, but it DOES bump `buffers.version()` so the `tabs` cache (which composes that version into its key for label resolution) correctly rebuilds.
...
### E.2 — Element-tree reuse [dropped — E.2.α analysis]

E.2.α (`f8aa713`) was scoped as an investigation pass to confirm that the renderer's element-tree construction was actually a cost worth attacking before scoping E.2.b–e implementation work. The findings closed the slice without a single sub-slice landing — each one had a concrete reason not to ship, all measurement-backed:

- **Notify cadence is input-driven.** `EditorView::render` is triggered by exactly four `cx.notify()` callers — the worker bridge (one wake per published `RenderState`), popup dismissal, `on_key_down`, and tabline click. No idle-fire path, no over-trigger. Rebuilding the element tree on every wake is the cost we *wanted*; the worry was that some host event was over-firing notify and producing zero-delta repaints. It isn't.
- **Conditional overlay-block construction is already correct.** Overlay subtrees (E/F/G/H/I — popup, picker, completion, hover, signature) `return None` and skip construction when their backing state is `None`. Steady-state (no overlay open) doesn't pay for any of them.
- **The four candidate sub-slices don't justify the implementation cost** against the prepaint baselines captured in `editor_element_frame` and the publish baselines captured in `dispatch_publish`:
  - **E.2.a / E.2.b / E.2.c** — promote popup / picker / completion overlay bodies to a shared `OverlayElement` that reuses the element subtree across frames. Per-char `div()` cells inside these overlays *are* re-allocated every frame they're visible, but they're only constructed while the overlay is open. The benches don't cover overlay-open frames (no `TestAppContext` on gpui 0.2.2 makes that hard to wire). Defer until someone runs a manual `profile-frames` trace against a popup-open session and finds it hot.
  - **E.2.d** — tabline `SharedString` identity reuse (cache an `Arc<str>` per tab keyed on `(buffer_name, idx)` instead of `SharedString::from(String::from(...))` per frame). Runs every frame regardless of focus, but the win is sub-µs at typical tab counts (≤ 8 tabs). The B.4.b `tabs` sub-state cache already preserves the published `TabsRenderState.items` Arc across no-op publishes, which collapses the upstream churn this slice would address. Defer.
  - **E.2.e** — consolidate the multiple `render_state.load*()` calls inside `EditorView::render` + `paint_pane` into one shared `RenderStateGuard`. Sub-µs cleanup; arguably a clarity win but not a perf win. Skip unless the duplicated reads start fighting borrow-checker friction on a future slice.

The slice's value was always going to be measured against the prepaint baseline that E.2.α captured: ~104 µs at viewport 120 with overlays applied — well inside the 8 ms keystroke→glyph budget. The architectural reuse the E.2 plan envisioned is not the missing piece.
