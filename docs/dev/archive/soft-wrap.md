...
| **W.6**  | ✅ `gj`/`gk`/`g0`/`g$` display-line motions (2026-06-07). AppEffect→Action→`do_*` chain; `goal_col: Option<u32>` on `Editor` (`w_curswant`); reset before every non-`gj`/`gk` action; `active_wrap_width()` reads `option_cache.wrap_lines` + published `CellMatrix`. Bindings at `[g,j]`/`[g,Down]`, `[g,k]`/`[g,Up]`, `[g,0]`, `[g,$]` in `keymap_normal.rs`; `I` / `A` also landed here. 8 tests in `dispatch.rs` covering wrap-off/wrap-on single/multi-segment paths. | ✅ |
| **W.7**  | ✅ `:set wrap` live-toggle + width-change rebuild wiring (2026-06-07). Infrastructure already complete: `publish_render_state()` fires `cells_wake` unconditionally; `PaneCellsInputs.wrap` reads `option_cache.wrap_lines`; existing cache-hit guard (`existing.wrap_width != new_wrap_width`) invalidates on both changes. Four tests in `dispatch.rs`: toggle/invalidation/viewport-width-change/goal-col-clear. `↪` (U+21AA) BMP fallback already shipped in W.4. | ✅ |
