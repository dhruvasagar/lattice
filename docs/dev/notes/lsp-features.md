...
| `workspace/didChangeWatchedFiles`     | 4.4.l | 🚧     | Wire wrapper shipped (`ServerHandle::did_change_watched_files`) + pure parse + match layer in `lattice-lsp::file_watcher`: `compile_with_workspace_root` walks the dynamic registry's `workspace/didChangeWatchedFiles` entries, compiles each `FileSystemWatcher` glob into one shared `globset::GlobSet`, and honours per-watcher `WatchKind` (default 7 = Create|Change|Delete). `WatcherSubscriptions::matches(path, kind)` returns the firing watcher indices per fs event. `fingerprint()` lets the dispatcher cache "did registrations change?" cheaply. The fs event source (notify-crate driver + debounce + fan-out to running actors) lands in 4.4.l.2.                                                                                                                                                                                                                                                                                                                  |
