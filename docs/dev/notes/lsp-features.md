...
| `window/showMessageRequest`       | 4.4.b | 🚧     | `ShowMessageRequestBus` + App-side drain shipped. Drain logs to `*lsp:<server>*`, surfaces the prompt + action labels to the minibuffer (with severity matching LSP level), and auto-replies `null` (dismissed). Picker-driven response UX queued for a follow-up; most servers use this rarely enough that "dismissed" is a reasonable default. |
...
| `textDocument/inlayHint`                 | 4.4.g | ✅     | `ServerHandle::inlay_hint` + `supports_inlay_hint` capability probe + `lsp-inlay-hint-mode` sub-mode. Per-tick pump (`maybe_request_inlay_hint`) fires on document-version change; drain seats `LspInlayHintCache` keyed by `(BufferId, doc_version)`. Renderer's `splice_virtual_text_into_spans` helper inserts each hint label at its utf-16→utf-8-converted byte offset, styled as italic + dim gray. `paddingLeft` / `paddingRight` honored. Whole-buffer range (viewport-only optimization queued). |
| `inlayHint/resolve`                      | 4.4   | ⏹️      | Lazy-resolve tooltip / textEdits when the hint gains focus. Servers must work with clients that don't resolve; deferred until the hint-tooltip UX lands.                                                                                                                                                                              |
...
- **4.4.b** -- 🚧 partial / mostly shipped. `$/setTrace`
  ...
  `:e`; `external` delegates to OS handler) shipped. The
  `window/showMessageRequest` wire path is also live (bus +
  dispatch + App-side drain) but the user-facing UX is
  "auto-dismiss with prompt visible in minibuffer + LSP log";
  a proper picker-driven response UX is queued for a follow-up
  slice. None of the slice's wire surfaces require renderer
  changes.
