...
| 5.2            | 🟡 in progress | **Move pure HOST modules.** Everything classified HOST in the audit that doesn't transitively touch `theme.rs` or `chord.rs`. lattice-ui-tui re-exports the moved types via `pub use lattice_host::*;` so downstream consumers don't break. ~1-2 weeks, one or two modules per commit. **First wave shipped:** six trivial re-export shims migrated. **Second wave shipped:** `actions.rs` + `excommand.rs` migrated. **Third wave shipped:** `host_generators.rs`. **Fourth wave shipped (post-5.4):** the three keymap leaves -- `keymap.rs` (691 LoC), `keymap_trie.rs` (591 LoC), `keymap_registry.rs` (1261 LoC) -- migrated to `lattice-host`. lattice-host gains `arc-swap` workspace dep (`keymap_registry` reads its `ArcSwap`-backed snapshot lock-free). The `keymap_entry!` `#[macro_export]` macro is re-exported at lattice-ui-tui's root so the catalog files (still in lattice-ui-tui until Action moves) continue to invoke it unqualified. **Blocked next:** the keymap catalogs (`keymap_normal/insert/visual/replace.rs`, ~4,600 LoC combined) import `crate::app::Action` -- needs the Action enum extracted from `app.rs` to lattice-host. That's a focused mini-slice on its own. 1472 ui-tui + 127 lattice-host = 1599. |
