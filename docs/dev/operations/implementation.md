...
| S4.final.b paint_cells path | ⛔ planned | `paint_cells(matrix_slice, …)` emits per-cell `paint_glyph` + bg `paint_quad`; wired into `EditorElement::paint` behind a runtime toggle; both `shape_line` and `paint_cells` paths coexist until S4.final.f. |
| S4.final.c cell-grid hit-test | ⛔ planned | Replace `ShapedLine::closest_index_for_x` with a cumulative-advance walk over `CellRow.cells`. Click + cursor positioning land on the cell grid. |
| S4.final.d modifier paint | ⛔ planned | UNDERLINE / DIM / REVERSE rendering on the paint_cells path. Visual parity with the TUI cells path. |
| S4.final.e emoji + fallback | ⛔ planned | Cells whose primary-font glyph id is `0` (.notdef) fall back through `layout_line` + `paint_emoji`. Resolved `(font_id, glyph_id, is_emoji)` triple is cached per-codepoint. |
| S4.final.f retire shape_line | ⛔ planned | `EditorElement`'s legacy `shape_row` / `build_line_with_inlays` / `LineRunBuilder` / `make_run_with_color` delete; `paint_cells` becomes the only document-body path. `cells_paint::cell_row_to_text_runs` retires alongside it. Gutter + status / popup / picker / help / synthetic buffers keep their Shaped paths. |
