...
| 5.8+           | ⛔     | **GPUI feature parity, incrementally.** Input adapter (GPUI key event → `KeyChord`), text paint, panes, pickers, popups, syntax highlighting, completion popup. Each is a slice of its own; TUI keeps working at every commit. |
