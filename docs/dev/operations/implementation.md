...
| 5     | GPU Rendering Foundation              | 🟡 in progress (5.0–5.4 ✅; 5.B substantively done; 5.5+ open) | Phase 5 splits the host out of `lattice-ui-tui` so GPUI can land as a peer renderer. Plan: [`../architecture/phase-5-extraction.md`](../architecture/phase-5-extraction.md). **Status:** ~24k LoC migrated from `lattice-ui-tui` to `lattice-host`/`lattice-core`. 5.0 audit ✅, 5.1 empty crate ✅, 5.3 renderer-neutral theme ✅, 5.4 chord split ✅ (input.rs split deferred). 5.2 HOST modules ✅ except keymap catalogs (MIXED, blocked on the dispatch refactor that also lands the deferred 5.4 input.rs split). **5.B App→Editor composition** ([`../architecture/phase-5b-app-design.md`](../architecture/phase-5b-app-design.md)): pivoted from Option D (generics) to Option E (composition) in 5.B.3; clusters 1–12 from the plan shipped across 5.B.4 → 5.B.19 plus a C1–C5 wave (cmdline completion, visible-highlights, popup snapshot, pane_tree, document/snapshot_cache, folds, LSP file watcher). `App` shrank from ~6k → 4,151 LoC; `Editor` is 738 LoC. **5.B tail still open**: completion cluster (#8) — `insert_completion` + 7 siblings still on `App`; `popup_back_stack` (popup #7 tail); `pending_config_structural_sections`. **5.5–5.last**: pane render trait, `lattice-render` crate, `--tui` flag, GPUI scaffold — gated on closing 5.B's tail. |
...
| 5.2            | 🟡 in progress | **Move pure HOST modules.** Everything classified HOST in the audit that doesn't transitively touch `theme.rs` or `chord.rs`. lattice-ui-tui re-exports the moved types via `pub use lattice_host::*;` so downstream consumers don't break. **Waves shipped:** trivial shims; actions + excommand; host_generators; keymap leaves (keymap.rs, keymap_trie.rs, keymap_registry.rs). **Action extracted** (post-keymap-leaves): the `Action` enum + `FindKind` + `EchoMessage` + `EchoLevel` move to `lattice_host::action`. `lattice-ui-tui::app` re-exports them. Combined: 1472 ui-tui + 127 lattice-host = 1599. **Catalog migration reclassified MIXED, blocked**: the four `keymap_normal/insert/visual/replace.rs` files (~4,600 LoC) contain `dispatch_*(handle, &KeyEvent, ...)` functions that match on `crossterm::KeyCode` / `KeyModifiers` directly -- they're MIXED, not pure HOST. Unblocking requires refactoring the dispatch functions to take a renderer-neutral `KeyChord` argument (lattice-host owns the data + the host dispatch; lattice-ui-tui's runtime converts `KeyEvent → KeyChord` before calling host dispatch). That's a focused refactor of its own and is deferred -- the dispatch refactor benefits multiple subsystems (input.rs likewise becomes a thin crossterm bridge), and bundling it with the catalog move lets it land coherently. |
...
| 5.4            | 🟡 in progress | **Split `chord.rs` + `input.rs`.** Renderer-neutral `KeyChord` + `dispatch_chord(ctx, chord) -> Option<Action>` move to host. Crossterm adapter `KeyChord::from_crossterm(&KeyEvent)` + `translate_event` shim stay in TUI. **chord.rs split shipped:** all renderer-neutral types (`KeyChord`, `KeyKind`, `KeyMods`, `SpecialKey`, `ChordParseError`, `parse_chord_sequence`, `last_chord_token_byte_len`, `special_label`, `Display`/`FromStr` impls) migrated to `lattice-host::chord`. The crossterm adapter (`from_event`, `format_chord`) stays in `lattice-ui-tui::chord` as **free functions** -- orphan rules prevent extending `KeyChord` from the renderer crate. Existing `KeyChord::from_event(&ev)` call sites rewrite to `chord::from_event(&ev)` (6 production sites across keymap_visual/insert/normal/replace + 4 bench sites). The crossterm test suite (~13 tests) stays in lattice-ui-tui; the neutral parse/Display/round-trip tests (~14) move to lattice-host. 1521 ui-tui + 78 lattice-host = 1599. **input.rs split deferred** -- focused on chord first to unblock the keymap family migration; input.rs comes when the keymap family lands. |
| 5.B            | 🟡 tail | **App → Editor composition migration.** See [`../architecture/phase-5b-app-design.md`](../architecture/phase-5b-app-design.md). 5.B.0 audit (`f7416a1`), 5.B.1 host `Renderer` trait + `MinimalRenderer` (`6f651a4`), 5.B.2 `App<R>` generics (`3649c18`, reverted), 5.B.3 Option-E pivot (`3e0db86`) — `Editor` defined, `App.editor` field added. Clusters 1–12 from the plan landed across 5.B.4–5.B.19: macros (`8fc1d5b`), marks+registers (`58fb496`), position history + tag stack (`cedbaca`), search (`7a9fb4d`), vim repeat + visual (`1944035`), replace+insert (`0879473`), popup 3/4 (`4c7bd1e`), cmdline+echo (`49bbe15`), syntax (`e9203e1`), picker (`f3d0652`), config+modes (`2eb61ad`), modal+dispatch (`37b4dbd`), active-pane subset (`94dcebb`), LSP per-buffer caches (`acf5b2a`), LSP scaffolding (`d00b4d9`), LSP subsystem + server channels (`f1ba0f9`), LSP request channels (`d5b5b82`). C-wave finished the keystone fields: C1 cmdline `CompletionState` + `completion_registry` (`0d84c6f`), C2/C3 `VisibleHighlightsKey` + `PopupSnapshot` to host (`bb4911e` + `e51db26`), C4a `pane_tree` (`ed8c4ec`), C4b/c `document` + `snapshot_cache` (`db31aeb`), C4 final `folds` (`c57346c`), C5 `LspFileWatcher` (`e51b4ed` + `2e3af79` + `be5c1d0` + `3465f6f` + `9401d4e`). `App` is now 4,151 LoC; `Editor` 738 LoC; workspace green. **Tail remaining:** completion cluster (#8) — `insert_completion`, `snippet_registry`, `insert_completion_snippet_meta`, `completion_accept_freq`, `per_language_completion`, `completion_in_path_context`, `active_snippet`, `snippet_dirs`; popup `popup_back_stack`; `pending_config_structural_sections`. |
