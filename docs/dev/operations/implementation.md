...
| 5     | GPU Rendering Foundation              | 🟡 in progress (5.0–5.4 ✅; 5.2 partial) | Phase 5 splits the host out of `lattice-ui-tui` so GPUI can land as a peer renderer. Plan: [`../architecture/phase-5-extraction.md`](../architecture/phase-5-extraction.md). **Status (after autonomous session):** ~17k LoC migrated from `lattice-ui-tui` to `lattice-host`/`lattice-core` across 14 commits. 5.0 audit ✅, 5.1 empty crate ✅, 5.3 renderer-neutral theme ✅, 5.4 chord split ✅. 5.2 multi-wave HOST module migration: shims + actions + excommand + host_generators + keymap leaves + Action enum + Fold + folds.rs + modes.rs + buffer_registry + pane shim ✅. **5.2 remaining**: keymap catalogs (MIXED, blocked on dispatch refactor), App struct + app/* submodules (the keystone). **5.5–5.last**: pane render trait, `lattice-render` crate, `--tui` flag, GPUI scaffold -- all gated on the App keystone. Test count steady at 1599 throughout. |
