...
| 5.9            | ⛔     | **`lattice-cli --gpu` / `--tui` flags.** Plumbed once GPUI runs end-to-end. Default stays TUI; `--gpu` is opt-in. |
...
| 3b.* | ⛔ planned | — | **Remaining LSP feature caches.** ~9 caches follow the 3b.1 template: `semantic_tokens`, `code_lens`, `document_link`, `document_color`, `pull_diagnostics`, `references`, `symbols`, `selection_range`, `completion` / `insert_completion` / `completion_resolve`. Each migration is mechanical: field type + maybe_request rewrite + drain deletion + LspRenderState wiring + renderer read switch + test seeder updates. |
...
| 3b.AD | ⛔ planned | — | **Active-document publisher.** `ActiveDocumentRenderState` populated with cursor, scroll, viewport, modal state, visual selection, buffer kind. Renderer reads switch from `editor.cursor` to `rs.active_document.cursor`. The largest read-path migration (touches every TUI/GPUI render call site that reads doc state). |
| 3c | ⛔ planned | — | **Editor on its own thread.** Final step. `Editor` runs on a dedicated host thread; channels replace `&mut Editor` references; the renderer becomes a pure `RenderState` reader. Per CLAUDE.md paramount goal #4: "Three-layer architecture (UI / Core / Plugins) communicating via typed message passing. Multi-threaded by construction." |

**Cumulative test sweep (after 3b.1):** 217 host + 1410 ui-tui + 21 ui-gpui green. The two new primitives (`render_state` + `per_buffer_cache`) carry 9 dedicated tests locking in the cross-thread / cross-clone invariants every later sub-slice depends on.
