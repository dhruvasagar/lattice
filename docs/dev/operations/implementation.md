...
| msg-mode.2   | ⛔     | Typed option `messages.filter: String`. Defaults to `info`. Live-editable via `:set messages.filter=...`; the change reconfigures the subscriber's filter. Accepts standard `tracing-subscriber` filter syntax (`editor=info,lsp=debug,grammar=trace`).                                                                                  |
| msg-mode.3   | ⛔     | Syntax highlighting. `messages-mode` contributes a line-local highlight provider keyed by the bracketed level. Computed at append-time per new line; theme entries: `messages.trace`, `messages.debug`, `messages.info`, `messages.warn`, `messages.error`.                                                                                |
