...
| S2.3 full cell content | 🔄 in progress | Syntax span → `cell.fg`. Inlay-hint splicing → cells + inlay_offsets. Fold elision. Theme palette wiring. Sub-sliced as S2.3.a (syntax + theme), S2.3.b (inlays), S2.3.c (folds). Natural sync point for S3 to start in parallel once all three land. |
