...
| M-async.5 | ⛔ | **`LspMode::on_activate` awaits initialize handshake.** Rewrite `LspMode::on_activate` to `.await` the supervisor's `initialize_for(buffer, path)` round-trip before returning the Guard. The mode's "active" state then genuinely means "LSP is ready to serve requests" -- hover / completion / format requests issued immediately after activation are serviceable, not silently no-op. Adds `LspSupervisorHandle::initialize_for(buffer, path) -> Pending<...>`. Error handling: initialize timeout, supervisor crash mid-handshake → `Err(ModeActivationError)` → `ModeActivationFailed` event → App rollback. Bench `benches/mode_activate.rs`: cold + warm activation latency, p99 target after measurement. Doc fragment describing the LspMode lifecycle + supervisor coupling. Tests: success path (initialize completes → Guard returned → mode active); failure path (timeout → ModeActivationFailed → rollback). |
