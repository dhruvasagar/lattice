...
| S3 TUI cutover | 🔄 in progress | TUI consumes `Arc<CellMatrix>` instead of building cells in-renderer. Sub-sliced as S3.a (substrate modifier bits), S3.b (TUI body consumer), S3.c (retire RowPrepaint for code-class buffers). |
...
| S3.b TUI body consumer | ⛔ planned | TUI helper `CellRow → Vec<Span<'static>>` honouring fg + modifier flags. Wired into `draw_buffer` for document buffers; falls back to RowPrepaint when matrix empty. Side-by-side validation tests. |
| S3.c retire RowPrepaint | ⛔ planned | Drop the highlights worker's RowPrepaint dependency for code-class buffers once S3.b is solid. RowPrepaint stays for markdown/help/popups. |
