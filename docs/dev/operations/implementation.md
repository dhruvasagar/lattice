...
| 3c.final | ⛔ planned | — | **`App.editor: Editor` → `App.editor_handle: EditorActorHandle`.** Mechanical substitution: `App::new` swaps `Editor::boot(document)` for `spawn_editor_actor(Editor::boot(document))`; every `app.editor.set_X(v)` becomes `app.editor_handle.set_X(v).unwrap()`; every `app.editor.apply(action)` becomes `app.editor_handle.send_action(action).unwrap()`. Read sites are already on `app.ad()` after the atomic series, so they don't change. Field-of-editor reads that survived the atomic series either get a render-state widening or a `Get<T>` synchronous actor command. |
