...
| S2 cell-builder worker | 🔄 in progress | Tokio worker in `lattice-host`, sibling of `highlights_worker`. Subscribes to (text, syntax, inlay, fold, theme) version cascade; coalesces; rebuilds intersecting chunks; publishes via ArcSwap on `RenderState`. Edits past the change point shift `start_source_line` without rebuilding. Internally sliced S2.1–S2.5. |
...
| S2.4 chunked mode | 🔄 in progress | Chunked mode + incremental rebuild. Sub-sliced as S2.4.a (structural switch) and S2.4.b (smallest-rebuild-set + downstream chunk shift; needs edit-range plumbing). |
| S2.4.a chunked-mode switch | ✅ landed | `pick_chunk_size(viewport_height, line_count)` returns `WholeDoc` when `viewport_height == 0` or `line_count ≤ 4 × viewport_height`, otherwise `Chunked(next_pow2(2 × viewport_height))` (clamped to a 16-line floor). `build_matrix` builds a `ChunkInputs<'_>` borrow bundle once (syntax spans, inlay buckets, fold index, theme) and a small `build_chunk_rows(&inputs, start, end)` helper produces each chunk's rows. Every chunk carries the same publisher `MatrixVersion` for now; S2.4.b will swap reused chunks via per-chunk version comparison. 6 tests cover the policy table, `next_power_of_two` edges (incl. `u32::MAX` saturation), small-doc whole-doc, large-doc chunk count + start_source_line + row content, fold elision across chunk boundaries, viewport-shrink-promotes-to-chunked. |
| S2.4.b smallest-rebuild-set | ⛔ planned | Threads `(start_line, lines_removed, lines_added)` edit deltas from the document into the cells substate. Worker compares per-chunk versions; chunks whose covered range is unaffected reuse their `Arc<CellChunk>`; chunks past the edit shift `start_source_line += Δ` without rebuild. |
| S2.5 coalescing | ⛔ planned | Debounce burst wakes. `paint_request` integration. Verify cell-builder + highlights workers don't fight. End-to-end smoke test. |
| S3 TUI cutover | ⛔ planned | TUI consumes `Arc<CellMatrix>` + `OverlayState` instead of building cells in-renderer. Validates substrate on the surface where cell-grid is already the native model. May start in parallel after S2.3. |
