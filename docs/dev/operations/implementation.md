...
| 5.2            | 🟡 in progress | **Move pure HOST modules.** Everything classified HOST in the audit that doesn't transitively touch `theme.rs` or `chord.rs`. lattice-ui-tui re-exports the moved types via `pub use lattice_host::*;` so downstream consumers don't break. ~1-2 weeks, one or two modules per commit. **First wave shipped:** six trivial re-export shims (`buffers`, `file_tree`, `oil`, `popup`, `help`, `help_topics`) migrated. These were pre-existing M.4 shims that already pointed to renderer-agnostic crates; moving them establishes the migration pattern (file moves to `lattice-host/src/`, `lattice-ui-tui/src/lib.rs` replaces `pub mod X;` with `pub use lattice_host::X;`) and proves the workspace seam works end-to-end. 1592 ui-tui tests still pass. |
