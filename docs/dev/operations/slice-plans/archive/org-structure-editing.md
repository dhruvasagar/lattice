...
| `press_chord(&mut Editor, &str)` | **exists** (~line 321) — **FIXED 2026-09-07** (`a28bf90c` host, `644afa9` plugin). It used to DOUBLE-APPLY a plugin action: `dispatch_chord` had already run it and the helper re-dispatched the resolved `Action::Invoke` to recover the effects. Measured in OS.4 — `<leader><CR>` inserted ONE item via `press`, TWO via `press_chord` — while the helper carried a comment claiming it avoided exactly that. The host now exposes `dispatch_chord_with_outcome`, so the effects come back from the single run that already happened. `press_chord_applies_an_editing_action_exactly_once` pins it. Either helper is now correct; `press_chord` additionally applies renderer effects |
