...
| **MO.4.a** | 🚫 Decoration migration | `DecorationProvider` is `_private:()` stub in `lattice-mode/src/contributions.rs`. Gutter-sign migration blocked on M.4 decoration registry. **Unblock:** M.4 decoration registry lands. |
...
| **MO.4.c** | 🚫 Subscription migration | `Subscription` is `_private:()` stub. Five host-side `subscribe_typed` calls in `editor_boot.rs` cannot be moved until the typed event-bus shape finalises. **Unblock:** `Subscription` type + event-bus shape finalised. |
