...
| **MO.2** | 🗒 Oil keymap migration | Single binding (`-` → `oil_navigate_up`). Move to a new `register_oil_mode_keymap(handle, action_ids)` helper in `lattice-oil/src/modes.rs`. Boot pushes under `PushLayerKind::MinorMode(OilMode::mode_id())`. Drop the entry from `keymap_normal.rs`. Touches: `lattice-oil/src/modes.rs`, `lattice-host/src/keymap_normal.rs`, `lattice-host/src/editor_boot.rs`. |
| **MO.3** | 🗒 Snippet keymap migration | Design call first: introduce a `snippet-active-mode` minor (separate from the always-on `snippet-mode` that contributes completion sources) representing "a snippet session is currently active on this buffer." Migrate 4 Insert-mode bindings (`<Tab>` expand, `<Tab>`/`<S-Tab>` next/prev placeholder, `<Esc>` leave) under its layer. Activate/deactivate alongside the existing `active_snippet` lifecycle. Drop the runtime `is_snippet_active` boolean check from K.1.c. Touches: `lattice-snippet/src/modes.rs`, `lattice-host/src/keymap_insert.rs`, `lattice-host/src/keymap_registry.rs` (drop the runtime check), the active-snippet enter/exit paths. |
| **MO.4** | 🗒 Decoration / subscription / status-line audit | Broader pass surfaced in mode-architecture §13. Audit candidates: gutter signs (diagnostics, diff hunks, LSP code-lens chips) → `Mode::decorations()` contributions; status-line items (LSP "rust-analyzer ready", diff "+5 -3", macro recording indicator) → new trait method (e.g. `Mode::status_line_items()` returning `Vec<StatusLineItem>`); LSP-specific typed-event subscriptions currently wired host-side (progress, diagnostic refresh, semantic tokens refresh, code-lens refresh, document highlight) → `LspMode::on_activate`'s returned Guard. Scoped per-area; can split into sub-slices (MO.4.a decorations / MO.4.b status-line / MO.4.c subscriptions) once the design pass identifies the seams. |
