...
| **M.6.2** | Dedup + cursor-placement + regex matching | Service-backed dedup so same-file hits across batches reuse the source buffer. `do_edit_with_row` variant (or post-edit cursor seek) so jump-to-source lands at the matched line. Switch literal matcher to `grep-matcher` + `aho-corasick` for regex support. |
