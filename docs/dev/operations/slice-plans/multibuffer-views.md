...
| **M.6.3** | Regex matching | Switch literal matcher to `grep-matcher` + `aho-corasick` for regex support. Configurable via `ProjectSearchOptions::regex: Option<RegexFlavor>`. Default stays literal for back-compat. |
