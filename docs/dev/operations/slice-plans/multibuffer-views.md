...
| **M.6.5** | Headerline renderer adoption (queued — surfaced 2026-06-01) | `MultibufferDocumentHandle::set_headerline` (M.4) stores `HeaderlineStatus::{Idle / InProgress / Complete / Failed}` and publishes `MultibufferHeaderlineChanged`. No UI slice draws the view-header virtual row yet, so the user can't see scan progress visually (count of files scanned, "Done in N ms", error string). Required: (1) a view-header virtual-row publication path from the headerline cell (likely a tiny `HeaderlineVirtualRowProvider` mirroring K.4.6's `MultibufferHeaderProvider` shape but anchored at row 0 with the headerline status text); (2) renderer styling for the headerline row (distinct background, progress glyph during `InProgress`); (3) test that pushes through Idle → InProgress → Complete and asserts the matrix updates. Aligned with `project_async_buffer_status_in_headerline` memory — async-buffer status surfaces via headerline, not status line or toast. Gated on K.4.6 (registration seam + per-pane matrix resolution land first; this rides the same pipes). |
