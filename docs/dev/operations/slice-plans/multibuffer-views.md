...
| **M.6.5** | Headerline renderer adoption — **two-stream status surface** (queued — scope expanded 2026-06-03) | The view-header virtual row (`HeaderlineVirtualRowProvider` mirroring K.4.6's `MultibufferHeaderProvider` shape, anchored at row 0) renders **two status streams** that compose on the same surface: <br><br> **(1) Provider status** (the M.4 design): from `MultibufferDocumentHandle::set_headerline(HeaderlineStatus::{Idle / InProgress { label, count } / Complete { summary } / Failed { reason }})`. Search shows `Scanning… 247 files` → `Done — 47 hits in 12 files (520ms)`; failures show `Failed — invalid regex`. <br><br> **(2) View-system status** (added 2026-06-03 from user-testing observation post-M.11): surfaces transient UX-relevant view state the user otherwise has no visibility into. Initial set: <br>  • `Building cells…` during the cells-worker's first matrix build after activation (M.11's compose-heavy multibuffer makes this a perceptible one-time cost — user-reported "initial delay" 2026-06-03). <br>  • `N edits pending source sync` while the M.11 source forwarder has unflushed edits. <br>  • `Sources stale — refresh?` when an external pane edited a source after the user started typing into the multibuffer (M.4.x merge-policy hook; placeholder marker in v1, smarter resolution in a follow-up slice). <br>  • Multibuffer dirty indicator (different from regular `[+]` since multibuffer save is multi-source — see M.11.b). <br><br> **Open design decisions when the slice starts:** <br>  • Single concatenated line ("Search: 47 hits • Building cells…") vs structured segments with distinct styling. Heuristic-mapped recommendation: structured segments — protects paramount #2 (each stream is a separate contribution point providers/system can extend without colliding on a string-format protocol) and heuristic #1 (long-term fit: future streams — diagnostics, LSP request count — slot in cleanly). <br>  • Provider vs system priority on conflict. Default: system status is the LEFT segment (transient, attention-getting); provider is the RIGHT segment (steady state). `Building cells…` displaces nothing — appears alongside the provider segment. <br>  • Stale-state surface for external source edits. v1: simple `Sources stale` marker, no action affordance; v2 (M.4.x): keybinding to refresh + merge picker. <br><br> **Required artefacts:** <br>  • View-header virtual-row publisher reading both the headerline cell AND a new `ViewSystemStatus` cell on `MultibufferInner` (populated by the M.11 forwarder's queue depth + cells-worker `WorkerDecision` subscribers). <br>  • Renderer styling: distinct background (already in `host_theme` per theme convention), progress glyph during `InProgress` (nerd-font + BMP fallback per `feedback_icon_palette`). <br>  • TUI + GPUI updated in lockstep per `feedback_tui_gpui_parity`. <br>  • Test that pushes through Idle → InProgress → Complete provider stream AND independently `Building` → `Idle` system stream AND verifies they compose correctly when both are active. <br><br> Aligned with `project_async_buffer_status_in_headerline` memory — async-buffer status surfaces via headerline, not status line or toast. Gated on K.4.6 (registration seam + per-pane matrix resolution land first; this rides the same pipes). |
| **M.6.6** | Scan cancellation on refresh (queued — flagged in M.6.X retro) | `do_search_refresh` (M.6.1) spawns a fresh scan task via `spawn_scan_task`. The old task's `JoinHandle` is dropped by `attach_task`'s overwrite, but dropping the handle does NOT abort the task — the old scan keeps running on tokio's blocking pool until its `ignore::Walk` exits naturally. Stale results from the old scan can publish through `ProjectSearchBatchReady` and confuse the forwarder's view-id filter (mitigated today by the per-view filter, but wasted CPU + bandwidth). Required: a cancellation token threaded into `run_scan_blocking` checked at the top of each `walker.next()` iteration. Token flipped by `do_search_refresh` (and by the view's eventual `Drop` cleanup) before the new task starts. Test: assert that after refresh, the prior task observes the cancel and exits within a bounded number of iterations. |
| **M.6.7** | Per-excerpt syntax highlighting (cross-ref K.4.7) | See [K.4 slice plan §K.4.7](./multibuffer-is-a-regular-buffer.md#k47--per-excerpt-syntax-highlighting--design-slice). Excerpt body text currently renders unstyled because the multibuffer view's filename detects as `Lang::Plain`. Right design: each excerpt highlights using its **source buffer's** language (each source has a tree-sitter parse already; multibuffer slices the source's spans and shifts to composed coordinates). Architecture artefact `multibuffer-syntax.md` to be drafted under K.4.7.0 before code lands. Tracks here in the M-series ledger because the composer-side language tracking touches the multibuffer data model; the renderer-side resolution lives in K.4 territory. |
