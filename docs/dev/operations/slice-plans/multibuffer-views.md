...
| **M.6.1** | Source-load + jump-to-source + query refresh + bench | Open each hit's file as a Document buffer via `BufferStore::ensure_named_document`; append per-hit excerpts on the view (1-row excerpt at the hit row OR with ±2 context rows per design). Bind `<CR>` in `ProjectSearchMultibufferMode::keymap()` to jump-to-source. Bind `gr` to query-refresh chord (`r` alone would collide with vim's replace-character operator; `gr` is project-search-specific and unused by core grammar). Refresh cancels prior scan + spawns new (uses the existing `ProjectSearchService::attach_task` cancellation path). Ship `project_search_first_batch_p99_ms` ≤ 50ms / 1k-file bench gate in `lattice-multibuffer/benches/`. |
