...
| **M.4** | Live updates from source buffers + headerline API | Makes M.1's static composition live. Subscribes each multibuffer to its sources' `EventKind::DocumentChanged` events; on fire, schedules a debounced off-thread recompose + `ArcSwap::store` publish. **Source-close policy** (per design §8 + §3.7 lock): multibuffer publishes `MultibufferSourceClosed { view, source }` when a source closes; providers subscribe and choose policy. The Multibuffer itself does NOT auto-close unless every source is gone. **Anchor sliding**: introduce the `Anchor { line, col, generation }` type and swap `Excerpt::{start_line, end_line}` to anchor-based positions that update through the source's edit events. **Headerline status API** (per design §3.7 convention): `MultibufferDocumentHandle::set_headerline(HeaderlineStatus)` with `Idle / InProgress { label, count } / Complete { summary } / Failed { reason }`; publishes `MultibufferHeaderlineChanged { view }`. The view-header virtual row renders the status above the first excerpt (provider-agnostic; provider-minor pushes status updates). Refresh API surface: `refresh_for_source(source: BufferId)` hook on `DocumentChanged` self-loop debounced via version-number check. Tests: edit source outside any excerpt — multibuffer unchanged; edit inside an excerpt — composed view reflects; rapid edits coalesce; anchor below an insertion slides; source close publishes the typed event; headerline state transitions render correctly; self-loop debouncer skips multibuffer-originated changes. Bench: `multibuffer_source_edit_p99_us` ≤ 200µs at 1k excerpts. |
