...
| **M.4.1** | Anchor sliding + self-loop debouncer + bench | `Excerpt::{start_line, end_line}` swap to anchor-based `(line, col, generation)` so excerpts shift with insertions/deletions above their range. Self-loop debouncer via version-number check (skip recompose when the multibuffer's own apply_edit caused the source change). `multibuffer_source_edit_p99_us` ≤ 200µs at 1k excerpts. |
