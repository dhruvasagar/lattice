...
| **M.2** | Excerpt rendering — sub-sliced              | Excerpt headers render as virtual rows + `]e` / `[e` / `]E` / `[E` motions land. Carved per design §3.6 (2026-05-31) — multibuffer concerns extract into a dedicated `lattice-multibuffer` crate; `MultibufferMode` becomes the **major mode** for `BufferKind::Multibuffer`; motions register through the mode's keymap with the typed handle reached via the mode's per-buffer context. See M.2.a / M.2.b.0 / M.2.b.1 / M.2.b.2 / M.2.b.3 / M.2.c below. |
...
| **M.2.b.0** | 🗒 Reconnaissance — mode + grammar APIs    | Read `lattice-mode` + `lattice-grammar` to verify the three integration points the major-mode design depends on: (1) typed per-buffer context on `MajorMode` / `Mode` trait, (2) `BufferKind` → major-mode activation trigger (so `BufferKind::Multibuffer` slots in symmetrically to `FileTree` / `Oil` / `Help`), (3) parameterized motion registration in `lattice-grammar` so motion handlers can read the active mode's context at dispatch time. Surface any infrastructure gaps before code starts; if any of the three needs extension, that's a prerequisite sub-slice. Output: a short note in this file recording what's available and what (if anything) needs to be added before M.2.b.1. |
| **M.2.b.1** | 🗒 Extract `lattice-multibuffer` crate     | New crate `crates/lattice-multibuffer`. Move from `lattice-runtime::multibuffer`: `Excerpt`, `ExcerptId`, `ExcerptHeader`, `ExcerptHeaderStyle`, `RowEntry`, `RowTranslation`, `MultibufferDocumentHandle`, `MultibufferError`. Move from `lattice-host::multibuffer`: `MultibufferHeaderProvider`, `multibuffer_header_provider_id`, `default_header_cells`, `compose_header_rows`. Add `BufferKind::Multibuffer` variant in `lattice-core`. Add `register(mode_registry, grammar)` entry point. Existing tests come with the move. No behavioural change; mechanical relocation. |
| **M.2.b.2** | 🗒 `MultibufferMode` as major mode          | Define `MultibufferMode` + `MultibufferModeContext` (holds `Arc<MultibufferDocumentHandle>`). Implement `MajorMode` (associated `Context = MultibufferModeContext`, `kind() = BufferKind::Multibuffer`). `on_activate` registers the header provider against the buffer's virtual_row_providers slot (moves the wiring out of host boot into the mode). `on_deactivate` is the symmetric tear-down. Bridge: when a `BufferKind::Multibuffer` buffer registers in `BufferRegistry`, the major-mode activation fires automatically through the existing mode infrastructure (same path `FileTree` / `Oil` use today). Tests: activation registers the provider; deactivation unregisters; per-buffer context exposes the typed handle. |
| **M.2.b.3** | 🗒 Excerpt motions in the grammar           | Register `]e` / `[e` / `]E` / `[E` as `lattice-grammar` motions with motion ids `multibuffer.next-excerpt-start` / `prev-excerpt-start` / `next-excerpt-end` / `prev-excerpt-end`. Bind in `MultibufferMode`'s keymap. Motion handlers compute new cursor row from current row + excerpt geometry (read through the mode's `MultibufferModeContext`). Pure functions, unit-testable without grammar plumbing. Tests: motion lands on first row of next excerpt; landing past last excerpt is a no-op (cursor unchanged); operator composition (`d]e` deletes through next excerpt start); count composition (`3]e` jumps three excerpts ahead). |
| **M.2.c** | 🗒 `multibuffer_render_p99_us` bench        | Criterion bench measuring composed-snapshot read + virtual-row interleave cost at 50 visible excerpts. Gate ≤ 200µs per the §7 performance posture. Lives in `lattice-multibuffer/benches/` for crate-local CI. |
...
| **M.4** | Live updates from source buffers            | Makes M.1's static composition live. Subscribes each multibuffer to its sources' `EventKind::DocumentChanged` events; on fire, schedules a debounced off-thread recompose + `ArcSwap::store` publish. **Source-close auto-remove** (per §8 design decision, originally listed under M.1 but lives here architecturally because it needs the same event-subscription infrastructure): on `EventKind::DocumentClosed` for a source, prune that source's excerpts from the multibuffer; if the multibuffer ends up with zero excerpts, fire `DocumentClosed` for the multibuffer itself. **Anchor sliding**: introduce the `Anchor { line, col, generation }` type and swap `Excerpt::{start_line, end_line}` to anchor-based positions that update through the source's edit events — so excerpts shift with insertions/deletions above their range instead of staying pinned to original line numbers. Cross-pane consistency: edit in source pane reflects in multibuffer pane on next snapshot. Tests: edit source buffer outside any excerpt — multibuffer unchanged; edit inside an excerpt — composed view reflects; rapid edits coalesce into one rebuild; anchor below an insertion slides down; closing a source prunes its excerpts. Bench: `multibuffer_source_edit_p99_us` ≤ 200µs at 1k excerpts. |
