...
| **M.2** | Excerpt rendering — paused after M.2.b.1; resumes after H-series | Excerpt headers render as virtual rows + `]e` / `[e` / `]E` / `[E` motions land. Carved per design §3.6 (2026-05-31) — multibuffer concerns extract into a dedicated `lattice-multibuffer` crate; `MultibufferMode` becomes the **major mode** for `BufferKind::Multibuffer`; motions register through the mode's keymap with the typed handle reached via the mode's per-buffer context. **Paused** after M.2.b.1 (2026-05-31) because architectural review showed M.2.b.2 / M.2.b.3 would couple `lattice-multibuffer` to host through patterns every future kind-specific feature would have to fight. The [H-series](kind-agnostic-buffers.md) lands the kind-agnostic infrastructure first; M.2.b.2+ then becomes a clean consumer. See M.2.a / M.2.b.0 / M.2.b.1 / M.2.b.2 / M.2.b.3 / M.2.c below. |
