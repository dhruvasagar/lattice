...
| **M.4.2** | Self-loop debouncer (deferred) | Skip recompose when the multibuffer's own `apply_edit` caused the source change. Track per-source version of last self-originated edit; on DocumentChanged, if version matches, skip. Lands when profiling shows the redundant recompose matters (current 27 µs / 1k excerpts is already well under budget). |
