...
|---------|---------------------------------------------|------------|
| **M.-0** | Bench prep — Document hot-path baselines   | Land `document_read_p99_us` + `document_edit_p99_us` as a zero-behaviour slice. The benches measure the inner `Document` struct's hot-path reads and writes — useful general-purpose infrastructure but **not load-bearing for M.0** under the Path B trait design (M.0 doesn't touch the inner struct; trait dispatch overhead lives at the handle layer). Kept because the inner Document remains performance-sensitive and any future change to it should clear the same gate. |
| **M.0** | `Document` trait at the handle layer        | Trait abstraction over today's `DocumentHandle` (Path B per design fragment §3.1, 2026-05-31). 1. Define `Document: Send + Sync + 'static` trait in `lattice-runtime` mirroring today's handle API (snapshot reads + `Pending<T>` writes + dispatch). 2. Rename `DocumentHandle` → `RopeDocumentHandle`. Add `impl Document for RopeDocumentHandle` that delegates to existing methods — most signatures match the trait verbatim because the handle is already `&self`-only with Arc-snapshot-backed reads. 3. Switch `Editor.document` and any other slot that today stores a regular-document handle to `Arc<dyn Document>` so dispatch / motion / render paths serve both regular and multibuffer documents without kind branching (honoring [[feedback_buffers_no_special_case]]). 4. **Remove `RopeDocumentHandle::replace(...)`** (today's `DocumentHandle::replace`). Slot replacement is the only mechanism for "the active document changes" — `do_edit` rewrites to `BufferRegistry::open(path) → BufferId → spawn fresh handle → editor.document = registry.handle_for(buffer_id)`. The previous buffer stays in `BufferRegistry`; old actor task exits when its handle drops. One uniform code path for `:edit foo` / `:edit bar` / regular ↔ multibuffer transitions / `:b N` switches. **Zero change** to the inner `Document` struct, `DocumentActor`, `PublishedSnapshot`, `DocumentSnapshot`, or the mpsc write path — the trait is a thin abstraction over what already exists. Correctness gate: existing test suite passes unchanged + dispatch_publish / snapshot-cache benches show no regression (vtable indirection ~1–3 ns, lost in noise against actor mpsc + frame budget). **Semantic change to flag in review:** `:edit path` now creates a fresh buffer rather than swapping content in-place (vim shortcut → emacs/everything-is-a-buffer-aligned semantic); old buffer remains reachable via `:bn` / `:b N` / `:ls`. No multibuffer code yet. Reviewed independently and merged as its own PR before M.1 starts. |
