...
| M.3.2.b | **First per-kind migration.** Migrate `HelpBuffer`'s mode-specific fields (`links`, `highlights`) to buffer-locals owned by `help-mode`. Demonstrates the migration pattern on the smallest buffer kind. | `lattice-mode`, `lattice-ui-tui` | Help buffers' link nav still works through the new local-keyed access; `:describe-buffer` shows the locals grouped by `help-mode`. |
| M.3.2.c | **Remaining per-kind migrations.** Migrate `DocumentEntry` (syntax handle, fold list, sync versions) to locals owned by language modes; `FileTreeBuffer.entries`/`root` to `file-tree-mode`; `OilBuffer.dir`/snapshot to `oil-mode`. Decide at end of slice whether `BufferStorage` retires entirely or stays as the universal-payload discriminator. | `lattice-mode`, `lattice-ui-tui`, `lattice-syntax` | Every kind-specific read goes through `entry.locals.get::<T>()`; the per-kind structs reduce to universal-payload-only or disappear. |
