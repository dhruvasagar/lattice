...
| M.3.2.c | **Remaining per-kind migrations + field retirement.** Migrate `DocumentEntry` (syntax handle, fold list, sync versions) to locals owned by language modes; `FileTreeBuffer.entries`/`root` to `file-tree-mode`; `OilBuffer.dir`/snapshot to `oil-mode`. App-level reader sites in `app.rs` (link_at, scroll_to_anchor, anchor lookup) flip to buffer_locals. Drop the `links` / `anchors` / `highlights` fields from `HelpBuffer` and the analogous fields from the other variants. Decide whether `BufferStorage` retires entirely or stays as the universal-payload discriminator. | `lattice-mode`, `lattice-ui-tui`, `lattice-syntax` | Every kind-specific read goes through `entry.locals.get::<T>()`; per-kind structs are minimal (cursor / scroll / id / universal payload only). |
