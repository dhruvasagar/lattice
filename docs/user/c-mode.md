---
summary: "c-mode: the major mode for C files (`.c`, `.h`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [c, languages]
---

# c-mode

The major mode for C. Activates automatically on `.c`, `.h`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-c`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/c/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/c/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/c/textobjects.scm` |
...
`.h` maps here rather than to [`cpp-mode`](help:cpp-mode). A header is ambiguous by extension alone; C is the safer default because C headers are valid in C++ builds far more often than the reverse. Set the mode explicitly with `:cpp-mode` when a `.h` is really C++.
