...
A core plugin's detailed documentation — its options, its keys, how to test it
— **ships inside the plugin**, not in this page and not in lattice's binary.
Each registers its own `:help` topic through the `help` seam, so:
...
That is deliberate, and it is the same rule that applies to any plugin you
install yourself: docs travel with the artefact that owns them, so removing a
plugin removes its manual and a plugin that failed to load leaves none behind.
See [`plugins`](help:plugins) for how the seam works if you are writing one.

These are typed, not linked, because a `help:` link in this page would be a
dead link for anyone who removed the plugin — and this page has to stay
truthful in that case, since it is the page that explains how to remove them.
