---
summary: "css-mode: the major mode for CSS files (`.css`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [css, languages]
---

# css-mode

The major mode for CSS. Activates automatically on `.css`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-css`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/css/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/css/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/css/textobjects.scm` |
...
CSS buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a CSS file with an unusual name
