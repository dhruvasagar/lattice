...
[`*compilation*`](help:compilation-mode) buffer (or any particular buffer)
...
| `:problems` | `:copen` | | Open the [`*problems*`](help:compilation-mode#the-problems-view) grouped view |
...
records a position in the [jump list](help:modal-editing), so `<C-o>`
returns you.

> These chords are **not** `[d` / `]d`, which hop between LSP
> diagnostics **in the current file** — a separate feature (see
> [LSP](help:lsp)). Error-list navigation is cross-file and list-based.
...
`]d` (current-file) and the `:diagnostics` picker (see [LSP](help:lsp)).
...
| **Group** | `:problems` (`:copen`) | The `*problems*` [multibuffer](help:multibuffer-mode): excerpts grouped per file, **editable in place** |
...
- [Compilation mode](help:compilation-mode) — running commands and the
...
- [Multibuffer](help:multibuffer-mode) — the excerpt-composition surface
