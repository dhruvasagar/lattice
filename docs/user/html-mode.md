---
summary: "html-mode: the major mode for HTML files (`.html`, `.htm`, `.xhtml`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [html, languages]
---

# html-mode

The major mode for HTML. Activates automatically on `.html`, `.htm`, `.xhtml`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-html`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/html/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/html/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/html/textobjects.scm` |
...
HTML buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a HTML file with an unusual name
