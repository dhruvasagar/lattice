---
summary: "java-mode: the major mode for Java files (`.java`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [java, languages]
---

# java-mode

The major mode for Java. Activates automatically on `.java`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-java`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/java/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/java/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/java/textobjects.scm` |
...
Java buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a Java file with an unusual name
