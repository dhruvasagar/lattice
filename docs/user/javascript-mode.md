---
summary: "javascript-mode: the major mode for JavaScript files (`.js`, `.mjs`, `.cjs`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [javascript, languages]
---

# javascript-mode

The major mode for JavaScript. Activates automatically on `.js`, `.mjs`, `.cjs`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-javascript`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/javascript/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/javascript/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/javascript/textobjects.scm` |
...
JavaScript buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a JavaScript file with an unusual name
