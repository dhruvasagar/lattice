---
summary: "json-mode: the major mode for JSON files (`.json`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [json, languages]
---

# json-mode

The major mode for JSON. Activates automatically on `.json`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-json`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/json/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/json/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/json/textobjects.scm` |
...
JSON buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a JSON file with an unusual name
