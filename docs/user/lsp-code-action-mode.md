---
summary: "lsp-code-action-mode: per-buffer gate for textDocument/codeAction — turn it off and the code-action list is empty for this buffer."
...
`textDocument/codeAction` — the request behind `:lsp-code-actions`.
...
Toggling it off means the code-action list is empty for this buffer. Nothing else about the attachment
...
excellent at completion and slow at codeAction can be kept, with
