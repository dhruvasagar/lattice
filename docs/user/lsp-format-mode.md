---
summary: "lsp-format-mode: per-buffer gate for textDocument/formatting and rangeFormatting — turn it off and `:lsp-format` reports no server for this buffer."
...
`textDocument/formatting and rangeFormatting` — the request behind `:lsp-format`.
...
Toggling it off means `:lsp-format` reports no server for this buffer. Nothing else about the attachment
...
excellent at completion and slow at formatting can be kept, with
