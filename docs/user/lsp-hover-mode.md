---
summary: "lsp-hover-mode: per-buffer gate for textDocument/hover — turn it off and `K` stops asking the server; the popup no longer appears for this buffer."
...
`textDocument/hover` — the request behind the hover popup `K` opens.
...
Toggling it off means `K` stops asking the server; the popup no longer appears for this buffer. Nothing else about the attachment
...
excellent at completion and slow at hover can be kept, with
