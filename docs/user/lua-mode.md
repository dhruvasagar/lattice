---
summary: "lua-mode: the major mode for Lua files (`.lua`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [lua, languages]
---

# lua-mode

The major mode for Lua. Activates automatically on `.lua`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-lua`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/lua/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/lua/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/lua/textobjects.scm` |
...
Lua buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a Lua file with an unusual name
