---
summary: "The picker: :picker <source> (files, grep, buffers, lines, outline, …) — one vertico-style fuzzy finder over every source, with typed marginalia columns and syntax-highlighted previews."
related: [ex:picker, ex:files, ex:recent, ex:buffer-picker, ex:colorscheme]
---
...
| `:colorscheme`            | Theme picker, with live preview (see [`themes`](help:themes)) |
...
[`completion`](help:completion) for that surface.
...
slots documented in [`file-tree-mode`](help:file-tree-mode). Picker
...
`:customize`. See [`options`](help:options).
...
- **LSP** — `gr` (references), `:lsp-symbols` (document symbols),
  `:lsp-workspace-symbol`, `:lsp-code-action`. See [`lsp`](help:lsp).
- **Themes** — bare `:colorscheme` opens the live-preview theme
  picker. See [`themes`](help:themes).
- **Buffers** — `:b` opens the buffer switcher; `:ls` / `:buffers`
  print a flat listing. See [`buffers`](help:buffers).
