---
summary: "ruby-mode: the major mode for Ruby files (`.rb`, `.ruby`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [ruby, languages]
---

# ruby-mode

The major mode for Ruby. Activates automatically on `.rb`, `.ruby`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-ruby`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/ruby/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/ruby/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/ruby/textobjects.scm` |
...
Ruby buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a Ruby file with an unusual name
