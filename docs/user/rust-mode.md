---
summary: "rust-mode: the major mode for Rust files (`.rs`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [rust, languages]
---

# rust-mode

The major mode for Rust. Activates automatically on `.rs`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-rust`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/rust/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/rust/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/rust/textobjects.scm` |
...
Rust buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a Rust file with an unusual name
