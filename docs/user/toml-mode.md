---
summary: "toml-mode: the major mode for TOML files (`.toml`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [toml, languages]
---

# toml-mode

The major mode for TOML. Activates automatically on `.toml`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-toml-ng`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/toml/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/toml/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/toml/textobjects.scm` |
...
TOML buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a TOML file with an unusual name
