---
summary: "typescript-mode: the major mode for TypeScript files (`.ts`, `.mts`, `.cts`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [typescript, languages]
---

# typescript-mode

The major mode for TypeScript. Activates automatically on `.ts`, `.mts`, `.cts`.
...
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/typescript/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/typescript/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/typescript/textobjects.scm` |
...
Uses the strict TypeScript grammar. JSX is a different dialect and lives in [`tsx-mode`](help:tsx-mode).
...
TypeScript buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a TypeScript file with an unusual name
