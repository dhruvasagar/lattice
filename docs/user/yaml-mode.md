---
summary: "yaml-mode: the major mode for YAML files (`.yaml`, `.yml`) — tree-sitter highlighting, folds, symbols, and text objects."
related: [yaml, languages]
---

# yaml-mode

The major mode for YAML. Activates automatically on `.yaml`, `.yml`.

## What it gives you

| Capability | Source |
|---|---|
| Syntax highlighting | `tree-sitter-yaml`'s highlight query |
| Folds (`zc` / `zo` / `zM` / `zR`) | `queries/yaml/folds.scm` |
| Symbols (outline, `:picker outline`) | `queries/yaml/symbols.scm` |
| Text objects (`af` / `if`, `ac` / `ic`, …) | `queries/yaml/textobjects.scm` |
...
YAML buffer resolves to its global or default value. Tab width,
...
Detection is by file extension, so a YAML file with an unusual name
