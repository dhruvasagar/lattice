...
| 8b    | Bundled plugins                       | 📝 design underway; no crate built | Curated set of first-party WASM Component plugins shipping with the editor binary -- LSP server manager (lighthouse), plugin manager, fuzzy-finder, project grep, git client, snippet engine, editing helpers, diff viewer, outline sidebar, format-on-save, test runner, markdown preview. Each crate lives at `plugins/<name>/` (fuzzy-finder ships; the rest not built). See ../architecture/design.md §5.5.6 for the strategy + the seven WIT prerequisites Phase 7 must expose. **Design landed (2026-07, branch `phase-8-plugin-loader`):** the first-wave fragments — [`plugin-auto-pair.md`](../architecture/plugin-auto-pair) (the trivial-first plugin: `auto` + `manual` pairing), [`lighthouse.md`](../architecture/lighthouse) (LSP server manager + its net/proc/task/register-server host-services extension), and — **promoted to v1** — [`plugin-treesitter-seam.md`](../architecture/plugin-treesitter-seam) (the foundational plugin↔tree-sitter query seam that unlocks structural plugins) — each with a slice plan under `slice-plans/`. auto-pair surfaced two more general host prereqs (a grammar-context `document` handle; declining/fall-through bindings). **BUILT (2026-07-20, `phase-8-plugin-loader`):** **the auto-pair epic is complete (AP.0.1→AP.4)** and it is the FIRST plugin shipped — as a **core plugin**, not compiled-in. Delivered: the grammar `document` handle (AP.0.1) + fall-through/`Effect::Declined` (AP.0.2); the **tree-sitter query seam TS.1–TS.3** ([`plugin-treesitter-seam.md`](../architecture/plugin-treesitter-seam) — `tree-snapshot`/`node`/`query`/`tree-cursor`, host-side predicate eval, the `enclosing` scope query at ~1.1 µs, capability-gated); auto-pair's `auto` + `manual` styles (AP.1–AP.3, manual = TS.3's first structural consumer); and the **plugin-manager redesign core track PM.1–PM.4** ([`plugin-manager.md`](../architecture/plugin-manager)): a runtime-root search path + boot discovery, `cargo xtask build-core-plugins` staging, and the manifest `default_mode` + `<id>.enabled` config gate — so auto-pair auto-pairs **out of the box** (no `include_bytes`, no toolchain, no init.rs). Remaining plugin-manager work = the **user track PM.5–PM.8** (use-package `require` + build-on-boot from git/local sources). **Note:** much of the *other* bundled functionality already exists **natively** (fuzzy-finder = `lattice-picker`, project grep, git diff = `lattice-diff`, snippet engine = `lattice-snippet`, outline = `:lsp-symbols`, plus `lattice-claude-code`) — none yet repackaged as WASM Component plugins. |
...
Active focus: **Phase 8 (Plugin manager) — editor-side loading landed**
on branch `phase-8-plugin-loader`: the `lattice-plugin-loader` crate +
on-disk discovery + the `:plugin-load/unload/reload` commands + the
`init.rs` config path + the buffer-backed `:plugins` manager view
(PL8.A–H), plus the full plugin-observability stack (PO.1–PO.5: the
boundary tracer, the gated hot-path grammar seam, the `*plugin-trace*`
buffer views, the live `plugin.trace-level` option, and `wasi:logging`
guest logging). **Phase 7 (Plugin Host) complete** (PH7.0–PH7.12).
**Phase 4 (LSP)** wind-down complete at the wire+host layer (4.1–4.5 ✅;
11 wire-only rows deferred post-1.0 / upstream); **Phase 5.8 GPUI**
landed the peer renderer. **First bundled plugin shipped (2026-07-20):**
the **auto-pair epic (AP.0.1→AP.4) is complete** — auto-pair is the first
**core plugin**, prebuilt + discovered + on by default, built on the new
**tree-sitter query seam (TS.1–TS.3)** and the **plugin-manager core
...
[`surround-mode.md`](../architecture/surround-mode);
slice plan: [`surround-mode.md`](slice-plans/surround-mode).
...
**Phase 7 (Plugin Host) is complete** — paramount goal #2
(extensibility) now has its substrate: the WASM Component Model host
runtime, the WIT API package, the capability/fuel/crash-isolation
model, and every extension seam, each exercised end-to-end by a guest
fixture. The natural next architectural step is **Phase 8 (the plugin
manager)**: editor-side loading, on-disk plugin discovery, the user
`init.rs` → WASM config path, and shipping modes / reference plugins
as components — i.e. wiring the finished runtime into the running
editor (`lattice-host` does not yet depend on `lattice-plugin-host`).
...
[`../architecture/modeline.md`](../architecture/modeline), with
sequencing in [`slice-plans/modeline.md`](slice-plans/modeline)
...
specified in
[`../architecture/preview-isolation.md`](../architecture/preview-isolation),
with sequencing in
[`slice-plans/preview-isolation.md`](slice-plans/preview-isolation)
...
Anchor: [`../archive/phase-5-extraction.md`](../archive/phase-5-extraction) (the audit + slice plan).
...
Anchor: [../architecture/design.md §5.7](../architecture/design) (the async runtime + threading section, expanded with subsections §5.7.1 through §5.7.6 describing the architectural shape).
...
Anchor: [`../architecture/cell-grid-renderer.md`](../architecture/cell-grid-renderer) + design.md §5.6 + paramount goal #1.
...
| System / clipboard `"+`, `"*`                  | ✅ OS clipboard wired (CB series)      | [clipboard.md](../architecture/clipboard) |
...
[`slice-plans/terminal-mode.md`](slice-plans/terminal-mode); design in
[`../architecture/terminal-mode.md`](../architecture/terminal-mode).
...
Design fragment:
[`../architecture/diff-system.md`](../architecture/diff-system).
Synopsis in
[`../architecture/design.md`](../architecture/design) §5.13.
...
- ✅ **D.0a** (2026-05-28) — Displacing virtual-row primitive
  ...
  [`../architecture/virtual-rows.md`](../architecture/virtual-rows);
...
- ✅ **D.2.c** (2026-05-29) — Routing + debounce + bus
  ...
  shape per the
  [`docs/dev/architecture/diff-system.md`](../architecture/diff-system)
  §3.4 redesign:
...
- ✅ **D.2.e** (2026-05-29) — Criterion bench landed at
  ...
  - **`recompute_blocking`** at 1k / 5k / 50k lines —
    ...
    The 5k case is right at the 8.3 ms one-frame keystroke ceiling at 120Hz,
...
- ✅ **D.3** — Inline overlay presentation. Carved during build
  ...
  - ✅ **D.3.f.0** (2026-05-29) — `FoldProvider` trait +
    ...
    in [`fold-architecture.md`](../architecture/fold-architecture)
    with the slice plan at
    [`archive/fold-architecture.md`](../archive/fold-architecture);
  ...
  - ✅ **D.3.f.2** (2026-05-29) — fold-recompute bench
    ...
    **3.1 µs** (2677× under the 8.3 ms one-frame keystroke ceiling);
    pathological 1000-hunk recompute is **144 µs** (57×
...
- ✅ **D.4** — Side-by-side two-way: `:diffsplit`,
  ...
  [`pane-groups.md`](../architecture/pane-groups)).
  Carve in [`../archive/pane-groups.md`](../archive/pane-groups).
...
- ✅ **D.6** (2026-05-31, closed) — Three-way merge: conflict
  ...
  [`archive/diff-system.md`](../archive/diff-system)
...
- ✅ **D.8** — `:diffthis` as buffer-group membership +
  ...
  - ✅ **D.8.a** (2026-05-31) — `compute_diff` engine
    ...
    see [n-way-diff-membership.md](../architecture/n-way-diff-membership)
...
Design fragment:
[`../architecture/multibuffer-views.md`](../architecture/multibuffer-views).
Synopsis in
[`../architecture/design.md`](../architecture/design) §5.14.
...
- 🚧 **M.2** — Excerpt rendering. Sub-sliced per design §3.6
  ...
  **major mode** for `BufferKind::Multibuffer`; motions
  ...
  - **M.2 paused 2026-05-31 → resumed 2026-06-01.** Honest
    ...
    closed at H.2 (H.3 deferred — see [`kind-agnostic-buffers.md`](../architecture/kind-agnostic-buffers)
    ...
    [`multibuffer-views.md`](../architecture/multibuffer-views)
  ...
  - ✅ **M.2.c** (landed 2026-06-01, commit `7a7793b`) —
    ...
    ~12 µs — all ~690× under the 8.3 ms one-frame ceiling at 120 Hz.
...
Design fragment:
[`../architecture/kind-agnostic-buffers.md`](../architecture/kind-agnostic-buffers).
Slice plan:
[`archive/kind-agnostic-buffers.md`](../archive/kind-agnostic-buffers).
...
- ⏸ **H.3** — **Deferred 2026-06-01.** Original design preserved
  ...
  introduced in [`multibuffer-views.md`](../architecture/multibuffer-views)
...
- ✅ **K.2** — Keymap substrate (mode-owned bindings).
  ...
  **K.2.6 + K.2.7 docs** (this slice) — `mode-architecture.md`
  ...
  Design at
  [`../architecture/keymap-architecture.md`](../architecture/keymap-architecture#11-mode-owned-keymap-contributions-substrate-gap)
  §11 (chain ergonomic surface in §11.2.1; table form in
  §11.2.2); convention update in
  [`../architecture/mode-architecture.md`](../architecture/mode-architecture#13-mode-owned-keymaps--contribution-debt-2026-06-01)
...
- ✅ **K.3** — Help-prefix bindings (`<C-h>` map,
  ...
  **K.3.4 doc artefacts** (this slice) — `keymap-architecture.md`
  ...
  Design at
  [`../architecture/keymap-architecture.md`](../architecture/keymap-architecture#12-help-prefix-bindings-c-h-map)
  §12; sequencing + commit refs at
  [`../archive/help-prefix.md`](../archive/help-prefix).
- ✅ **K.4** — Multibuffer-is-a-regular-buffer audit +
  ...
  [`../architecture/multibuffer-is-a-regular-buffer.md`](../architecture/multibuffer-is-a-regular-buffer);
  sequencing at [`slice-plans/archive/multibuffer-is-a-regular-buffer.md`](slice-plans/archive/multibuffer-is-a-regular-buffer).
...
- **Buffer-level Insert-mode completion** -- design spec at
  [`../architecture/insert-completion.md`](../architecture/insert-completion). 4.2.g.1
...
Design fragment:
[`../architecture/buffer-local-options.md`](../architecture/buffer-local-options).
Slice plan:
[`archive/buffer-local-options.md`](../archive/buffer-local-options).
...
Design fragments:
[`../architecture/narrow-mode.md`](../architecture/narrow-mode) (the `zn`
operator + the one-excerpt view) and
[`../architecture/tree-sitter-text-objects.md`](../architecture/tree-sitter-text-objects)
(the `af`/`ac`/`aC`/… objects). Slice plan:
[`slice-plans/archive/narrow-mode.md`](slice-plans/archive/narrow-mode).
...
[`../architecture/keymap-architecture.md`](../architecture/keymap-architecture)):
...
See also:
[`slice-plans/multibuffer-providers.md`](slice-plans/multibuffer-providers)
...
Design fragment:
[`../architecture/host-provider-boundary.md`](../architecture/host-provider-boundary).
Slice plan: [`slice-plans/host-provider-inversion.md`](slice-plans/host-provider-inversion).
Motivation: the recurring-drift cost analysed in
[`../architecture/comparison-zed.md`](../architecture/comparison-zed) §5.
...
Design: [`../architecture/dashboard.md`](../architecture/dashboard). Slice
plan: [`slice-plans/dashboard.md`](slice-plans/dashboard).
...
Design: [`../architecture/treesitter-motions.md`](../architecture/treesitter-motions).
Slice plan: [`slice-plans/archive/treesitter-motions.md`](slice-plans/archive/treesitter-motions).
