...
| 8b    | Bundled plugins                       | ⛔ not started           | Curated set of first-party WASM Component plugins shipping with the editor binary -- LSP server manager (lighthouse), plugin manager, fuzzy-finder, project grep, git client, snippet engine, editing helpers, diff viewer, outline sidebar, format-on-save, test runner, markdown preview. Each crate lives at `crates/lattice-plugin-<name>/`. See ../architecture/design.md §5.5.6 for the strategy + the seven WIT prerequisites Phase 7 must expose. Depends on Phase 7. **Note:** much of the *functionality* already exists **natively** (fuzzy-finder = `lattice-picker`, project grep, git diff = `lattice-diff`, snippet engine = `lattice-snippet`, outline = `:lsp-symbols`, plus the `lattice-claude-code` integration) — but none ship as the WASM Component plugins this phase envisions; that repackaging is the actual not-started work. |
...
Active focus: **Phase 7 (Plugin Host) complete** (PH7.0–PH7.12, host
runtime + WIT seams + capability/fuel/crash model + `fuzzy-finder`
validation + CI gates). **Phase 4 (LSP) wind-down complete** at the
wire+host layer (4.1–4.5 ✅; 11 wire-only rows with their trigger UX
strong-reason deferred to post-1.0 / upstream); **Phase 5.8 GPUI
polish + feature parity** landed the peer renderer. **Phase 8 (the
plugin manager: editor-side loading, on-disk discovery, `init.rs`,
and modes/reference-plugins as components) is on deck** — the runtime
exists; wiring it into the editor is the next architectural step.
...
Slice plan: [`slice-plans/treesitter-motions.md`](slice-plans/treesitter-motions).
