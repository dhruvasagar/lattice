...
| `surround-mode`       | vim-surround operators (`ds`, `cs`, `ys`, `S` in visual) — native built-in, always active on document buffers. See [`surround`](../surround/). |
...
a clickable link — `<CR>` on ``lsp-mode``
